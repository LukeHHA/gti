...
private:
  ...
  void assignCNames() {
    ...
    for (NativeRecord &record : records) {
      if (record.info->namespaceScope.empty()) {
        record.cName = record.declaration->name().lexeme;
        used.insert(record.cName);
      }
    }
    for (NativeRecord &record : records) {
      if (record.cName.empty()) {
        record.cName = encodedCRecordName(*record.info);
        while (!used.insert(record.cName).second) {
          record.cName += '_';
        }
      }
    }
  ...
  const NativeRecord &recordFor(ClassId id) const {
    const auto found = recordByClass.find(id);
    if (found == recordByClass.end()) {
      throw std::logic_error(
          "checked C ABI type is unavailable to native header");
    }
  ...
  std::string cppType(const SemanticType &type) const {
    ...
    case SemanticType::Class:
      return qualifiedCppName(*recordFor(type.classId).info);
  ...
  std::string cType(const SemanticType &type) const {
    ...
    case SemanticType::StringView:
      return "gti_c_string_view";
    case SemanticType::Class:
      return recordFor(type.classId).cName;
    case SemanticType::RawPointer: {
      if (type.arguments.size() != 1) {
        break;
      }
