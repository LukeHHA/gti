  ...
  void visitFunctionDecl(const FunctionDecl &stmt) override {
    ...
    if (stmt.body()) {
      ...
      if (mirBody != nullptr) {
        output << ' ';
        if (mirLeaf != nullptr) {
          emitMirScalarLeafBody(*mirLeaf);
        } else if (mirCfg != nullptr) {
          emitMirScalarCfgBody(*mirCfg);
        } else if (mirDirect != nullptr) {
          emitMirScalarDirectBody(*mirDirect);
        } else if (mirOwned != nullptr) {
...
private:
  ...
  selectedMirScalarLeaf(const FunctionDecl &function) const {
    ...
    if (!std::all_of(info->parameterTypes.begin(), info->parameterTypes.end(),
        ...
        !(info->returnType == SemanticType::Void ||
          isMirScalarLeafType(info->returnType))) {
      return nullptr;
    }
    ...
    if (hirInstance == nullptr || hirInstance->id != selected->id ||
      ...
      throw std::logic_error(
          "verified MIR function instance does not belong to the matching "
          "HIR snapshot");
    }
    if (function.parameters().size() != hirInstance->parameterBindings.size()) {
      throw std::logic_error(
          "eligible HIR function has an invalid parameter binding map");
    }
    for (std::size_t index = 0; index < function.parameters().size(); ++index) {
      const HirBindingId bindingId = hirInstance->parameterBindings[index];
      const auto binding = std::find_if(hirInstance->body.bindings.begin(),
                                        hirInstance->body.bindings.end(),
                                        [&](const HirBinding &candidate) {
                                          return candidate.id == bindingId;
                                        });
      const BindingInfo *semanticBinding =
          semantics.findBinding(function.parameters()[index]);
      if (binding == hirInstance->body.bindings.end() ||
          binding->parameter != &function.parameters()[index] ||
          semanticBinding == nullptr ||
          binding->info.symbol != semanticBinding->symbol ||
          binding->info.type != semanticBinding->type ||
          binding->info.access != semanticBinding->access ||
          !sameMirScalarLeafTraits(binding->info.traits,
                                   semanticBinding->traits)) {
        throw std::logic_error(
            "eligible HIR parameter binding does not match its source "
            "formal");
      }
    }

    if (selected->owner || selected->staticMember ||
      ...
      throw std::logic_error(
          "eligible MIR function metadata does not match its semantic "
          "declaration");
    }
    if (!isHirScalarLeafBody(*hirInstance)) {
      return nullptr;
    }
    if (!isMirScalarLeafBodyShape(*selected)) {
      throw std::logic_error(
          "eligible scalar-leaf source body is not represented by the closed "
          "verified MIR family");
    }
    if (!isMirScalarLeafBody(*selected, *hirInstance)) {
      throw std::logic_error(
          "verified MIR scalar-leaf body does not match its HIR source or "
  ...
  [[nodiscard]] bool
  isHirScalarDirectNodeHeader(const HirFunctionInstance &hirFunction) const {
    if (hirFunction.source == nullptr || !hirFunction.source->body() ||
        hirFunction.source->runtimeBinding()) {
      return false;
    }
    const FunctionDecl &source = *hirFunction.source;
    const FunctionInfo *info = semantics.findFunction(source);
    if (info == nullptr || info->declaration != &source ||
        source.hasCLinkage() || source.operatorName() ||
        source.requiresClause() || !source.genericParameters().empty() ||
        info->id == 0 || !info->genericParameters.empty() ||
        !info->requirements.empty() || info->parameterPack ||
        info->ownerClass != 0 || info->entryPoint ||
        info->entryKind != ProgramEntryKind::None || info->staticMember ||
        ...
        !(info->returnType == SemanticType::Void ||
          isMirScalarCfgType(info->returnType)) ||
        hirFunction.id == 0 || hirFunction.declaration != info->id ||
        hirFunction.owner || !hirFunction.typeArguments.empty() ||
        !hirFunction.valueArguments.empty() ||
        hirFunction.returnType != info->returnType ||
        hirFunction.parameterTypes != info->parameterTypes ||
        hirFunction.entryKind != ProgramEntryKind::None ||
        hirFunction.entryArgumentAppendTarget || hirFunction.staticMember ||
        hirFunction.internalLinkage || hirFunction.constexprFunction ||
        hirFunction.returnBorrowOrigin != BorrowOriginKind::None ||
        hirFunction.returnBorrowParameter != 0 ||
        hirFunction.returnBorrowAccess != AccessMode::ReadOnly ||
        hirFunction.returnBorrowPlace ||
        hirFunction.linkage != LanguageLinkage::Gti ||
        !hirFunction.externalSymbol.empty() || hirFunction.virtualMethod ||
        hirFunction.pureVirtual || hirFunction.overrideMethod ||
        !hirFunction.virtualRoots.empty() ||
        !hirFunction.callableParameters.empty() ||
        source.parameters().size() != hirFunction.parameterBindings.size()) {
      return false;
    }
    for (std::size_t index = 0; index < source.parameters().size(); ++index) {
      const HirBindingId bindingId = hirFunction.parameterBindings[index];
      const auto binding = std::find_if(hirFunction.body.bindings.begin(),
                                        hirFunction.body.bindings.end(),
                                        [&](const HirBinding &candidate) {
                                          return candidate.id == bindingId;
                                        });
      const BindingInfo *semanticBinding =
          semantics.findBinding(source.parameters()[index]);
      if (binding == hirFunction.body.bindings.end() ||
          binding->parameter != &source.parameters()[index] ||
          semanticBinding == nullptr ||
          binding->info.symbol != semanticBinding->symbol ||
          binding->info.type != semanticBinding->type ||
          binding->info.access != semanticBinding->access ||
  ...
  [[nodiscard]] bool
  isMirScalarDirectNodeHeader(const MirFunctionInstance &function,
                              const HirFunctionInstance &hirFunction) const {
    if (!isHirScalarDirectNodeHeader(hirFunction)) {
      return false;
    }
    const FunctionDecl &source = *hirFunction.source;
    const FunctionInfo *info = semantics.findFunction(source);
    if (info == nullptr || hirFunction.id != function.id ||
        hirFunction.declaration != function.declaration ||
        hirFunction.declaration != info->id || hirFunction.owner ||
        !hirFunction.typeArguments.empty() ||
        !hirFunction.valueArguments.empty() ||
        hirFunction.returnType != function.returnType ||
        hirFunction.returnType != info->returnType ||
        hirFunction.parameterTypes != function.parameterTypes ||
        hirFunction.parameterTypes != info->parameterTypes ||
        hirFunction.parameterBindings != function.parameterBindings ||
        hirFunction.entryKind != function.entryKind ||
        hirFunction.entryArgumentAppendTarget !=
            function.entryArgumentAppendTarget ||
        hirFunction.staticMember != function.staticMember ||
        hirFunction.internalLinkage ||
        hirFunction.constexprFunction != function.constexprFunction ||
        hirFunction.returnBorrowOrigin != function.returnBorrowOrigin ||
        hirFunction.returnBorrowParameter != function.returnBorrowParameter ||
        hirFunction.returnBorrowAccess != function.returnBorrowAccess ||
        hirFunction.returnBorrowPlace != function.returnBorrowPlace ||
        hirFunction.linkage != function.linkage ||
        hirFunction.externalSymbol != function.externalSymbol ||
        hirFunction.virtualMethod != function.virtualMethod ||
        hirFunction.pureVirtual != function.pureVirtual ||
        hirFunction.overrideMethod != function.overrideMethod ||
        hirFunction.virtualRoots != function.virtualRoots ||
        !hirFunction.callableParameters.empty() ||
        hirFunction.body.placeDomain != function.body.placeDomain ||
        function.owner || function.staticMember ||
        function.receiverMutability != ReceiverMutability::ReadOnly ||
        function.overloadedOperator || function.constexprFunction ||
        function.definitionKind !=
            MirFunctionInstance::DefinitionKind::Source ||
        function.entryKind != ProgramEntryKind::None ||
        function.entryArgumentAppendTarget ||
        function.returnBorrowOrigin != BorrowOriginKind::None ||
        function.returnBorrowParameter != 0 ||
        function.returnBorrowAccess != AccessMode::ReadOnly ||
        function.returnBorrowPlace ||
        ...
        !function.virtualRoots.empty() ||
        !function.callableParameters.empty() ||
  ...
  [[nodiscard]] static bool isHirScalarDirectCallee(const HirValue &value) {
    return (value.kind == HirValueKind::Variable ||
            value.kind == HirValueKind::QualifiedName) &&
           value.source != nullptr &&
           value.unsafeOperation == UnsafeOperationKind::None &&
           value.operands.empty() && value.parameterTypes.empty() &&
           !value.operation && !value.literal && !value.constant &&
           value.intrinsic == IntrinsicKind::None &&
           value.synchronization.kind == SynchronizationOperationKind::None &&
           value.definedFailure.empty() &&
           value.borrowOrigin == BorrowOriginKind::None &&
  ...
  [[nodiscard]] bool isHirScalarDirectCall(const HirValue &value,
                                           const HirBody &body) const {
    if (value.kind != HirValueKind::Call || value.source == nullptr ||
        value.unsafeOperation != UnsafeOperationKind::None || value.operation ||
        value.literal || value.constant ||
        value.intrinsic != IntrinsicKind::None ||
        value.synchronization.kind != SynchronizationOperationKind::None ||
        ...
        value.packFoldSymbol != 0 || value.packFoldParameter != 0 ||
        value.packFoldFunction != 0 || value.packFoldArgument != 0 ||
        !value.packFoldElements.empty() || !value.functionTarget ||
        value.contextualBoolTarget || value.constructorTarget ||
        value.constructorKind != ConstructorKind::Ordinary ||
        value.lambdaTarget || !value.callableArguments.empty() ||
        value.callableBoundary || value.callableInvocation || value.enumOwner ||
        value.enumValue || value.enumVariant || value.payloadIndex ||
        value.ownership || value.dropObligation) {
      return false;
    }
    const HirFunctionInstance *hirTarget =
        hir.findFunctionInstance(*value.functionTarget);
    const auto *call = dynamic_cast<const Call *>(value.source);
    const ResolvedCallInfo *resolved =
        call == nullptr ? nullptr : semantics.findCall(*call);
    if (hirTarget == nullptr || hirTarget->source == nullptr ||
        resolved == nullptr || resolved->function != hirTarget->declaration ||
        resolved->declaration != hirTarget->source ||
        resolved->dispatch != CallDispatch::Static ||
        resolved->intrinsic != IntrinsicKind::None ||
        !resolved->typeArguments.empty() || !resolved->valueArguments.empty() ||
        !resolved->requirements.empty() ||
        resolved->returnType != value.info.type ||
        resolved->parameterTypes != value.parameterTypes ||
        !isHirScalarDirectNodeHeader(*hirTarget) ||
        hirTarget->returnType != value.info.type ||
        hirTarget->parameterTypes != value.parameterTypes ||
    ...
    if (callee == nullptr || !isHirScalarDirectCallee(*callee) ||
        callee->source != call->callee().get() ||
        call->arguments().size() != value.parameterTypes.size()) {
      return false;
    }
    for (std::size_t index = 0; index < value.parameterTypes.size(); ++index) {
      const HirCallArgument &argument = value.callPlan->arguments[index];
      const HirValue *source = body.findValue(argument.value);
      if (argument.parameterIndex != index ||
          argument.value != value.operands[index + 1] ||
          argument.parameterType != value.parameterTypes[index] ||
          argument.kind != HirCallInputKind::Value || source == nullptr ||
          source->source != call->arguments()[index].get() ||
          source->info.type != argument.parameterType ||
          !isMirScalarCfgType(source->info.type) ||
          !isMirScalarCfgTraits(source->info.traits, source->info.type)) {
        return false;
      }
    }
    return true;
  ...
  [[nodiscard]] bool isHirScalarDirectBody(const HirFunctionInstance &function,
                                           bool requireCall) const {
    const HirBody &body = function.body;
    ...
    return (!requireCall || calls != 0) && body.loans.empty() &&
           body.dropObligations.empty() &&
           std::all_of(body.bindings.begin(), body.bindings.end(),
                       [](const HirBinding &binding) {
                         return isMirScalarCfgType(binding.info.type) &&
                                isMirScalarCfgTraits(binding.info.traits,
  ...
  [[nodiscard]] static bool
  isMirScalarDirectCallInput(const MirInstruction &instruction,
                             const MirFunctionInstance &function,
                             MirBlockId block) {
    const MirBody &body = function.body;
    const MirValue *sourceValue =
        instruction.operands.size() == 1 &&
            ...
            ? body.findValue(instruction.operands.front().value)
            : nullptr;
    return instruction.kind == MirInstructionKind::CallInput &&
           ...
           sourceValue->sourceValue == instruction.hirValue &&
           isMirScalarCfgOperand(instruction.operands.front(), body) &&
           isMirScalarCfgResult(instruction, body, block);
  }
  ...
  [[nodiscard]] static bool
  isMirScalarDirectCall(const MirInstruction &instruction,
                        const MirFunctionInstance &function,
                        const MirProgram &program, MirBlockId block) {
    const MirBody &body = function.body;
    ...
    if (instruction.kind != MirInstructionKind::Call ||
        ...
        instruction.constructorInitializer != 0 || instruction.callInputRole ||
        instruction.callInputIndex != 0 ||
        instruction.callInputKind != HirCallInputKind::Value ||
        instruction.preparedParameterDrop || instruction.successResultDrop ||
        instruction.unsafeOperation != UnsafeOperationKind::None ||
        instruction.rawMemoryAccess || instruction.destination ||
        instruction.receiver || instruction.closureCaptureTypes.size() != 0 ||
        !instruction.closureCaptureModes.empty() ||
        instruction.packFoldSymbol != 0 || instruction.packFoldParameter != 0 ||
        instruction.packFoldFunction != 0 ||
        instruction.packFoldArgument != 0 ||
        !instruction.packFoldFixedPlaces.empty() ||
        !instruction.packFoldElements.empty() || instruction.loan ||
        ...
        instruction.dispatch != CallDispatch::Static ||
        instruction.dispatchOwner != SemanticType::Unknown ||
        target == nullptr || instruction.constructorTarget ||
        instruction.constructorKind != ConstructorKind::Ordinary ||
        instruction.lambdaTarget || !instruction.callableArguments.empty() ||
        instruction.callableBoundary || instruction.callableInvocation ||
        !instruction.lifecycle.empty() || instruction.ownership ||
        ...
        !(target->returnType == SemanticType::Void ||
          isMirScalarCfgTraits(instruction.info.traits,
                               instruction.info.type)) ||
        (target->returnType == SemanticType::Void
    ...
    for (std::size_t index = 0; index < instruction.operands.size(); ++index) {
      ...
      const MirInstruction *input =
          value == nullptr ? nullptr : mirScalarDefinitionFor(body, *value);
      if (input == nullptr ||
          !isMirScalarDirectCallInput(*input, function,
                                      value->definitionBlock) ||
          input->callSite != instruction.callSite ||
          input->callInputIndex != index ||
          operand.type != target->parameterTypes[index] ||
          input->info.type != target->parameterTypes[index]) {
        return false;
      }
    }
    return true;
  ...
  [[nodiscard]] static bool
  isMirScalarDirectBodyShape(const MirFunctionInstance &function,
                             const MirProgram &program) {
    const MirBody &body = function.body;
    if (function.mayRaiseDefinedFailure ||
        function.definitionKind !=
            MirFunctionInstance::DefinitionKind::Source ||
        body.kind != MirBodyKind::Function || body.entry == 0 ||
        body.blocks.empty() || body.entry > body.blocks.size() ||
        body.returnType != function.returnType || !body.loans.empty() ||
        !body.dropObligations.empty() || !body.cleanupBoundaries.empty() ||
        !body.failureRecords.empty() ||
  ...
  [[nodiscard]] bool
  isMirScalarDirectBody(const MirFunctionInstance &function,
                        const HirFunctionInstance &hirFunction) const {
    if (mir == nullptr || !isMirScalarDirectBodyShape(function, *mir) ||
        function.body.placeDomain != hirFunction.body.placeDomain ||
        function.body.fullExpressions.size() !=
            hirFunction.body.fullExpressions.size()) {
      return false;
    }
    for (std::size_t index = 0; index < function.body.fullExpressions.size();
         ++index) {
      const MirFullExpression &lowered = function.body.fullExpressions[index];
      const HirFullExpression &source = hirFunction.body.fullExpressions[index];
      if (lowered.id != index + 1 || lowered.hirExpression != source.id ||
          lowered.statement != source.statement ||
          lowered.constructorInitializer != source.constructorInitializer ||
    ...
    for (const MirPlace &place : function.body.places) {
      if (place.root == MirPlaceRootKind::Binding) {
        const auto binding = std::find_if(
            hirFunction.body.bindings.begin(), hirFunction.body.bindings.end(),
            [&](const HirBinding &candidate) {
              return candidate.id == place.binding;
            });
        if (binding == hirFunction.body.bindings.end() ||
            ...
            binding->info.type != place.type ||
            binding->info.access != place.access ||
            place.key != PlaceKey{.domain = function.body.placeDomain,
                                  .root = binding->info.symbol} ||
            !sameMirScalarLeafTraits(binding->info.traits, place.traits)) {
          return false;
        }
      } else {
        const HirValue *source = hirFunction.body.findValue(place.sourceValue);
    ...
    for (const MirValue &value : function.body.values) {
      const HirValue *source = hirFunction.body.findValue(value.sourceValue);
      const MirInstruction *definition =
          function.body.findBlock(value.definitionBlock) == nullptr
              ? nullptr
              : mirScalarDefinitionFor(function.body, value);
      if (source == nullptr || source->info.type != value.info.type ||
          !sameMirScalarLeafTraits(source->info.traits, value.info.traits) ||
          (definition != nullptr &&
    ...
    for (const MirBlock &block : function.body.blocks) {
      ...
      for (const MirInstruction &instruction : block.instructions) {
        ...
        for (std::size_t index = 0; index < instruction.operands.size();
             ++index) {
          const MirValue *operand =
              function.body.findValue(instruction.operands[index].value);
          const MirInstruction *input =
              operand == nullptr
                  ? nullptr
                  : mirScalarDefinitionFor(function.body, *operand);
          if (input == nullptr ||
    ...
    return std::all_of(exactCallCounts.begin(), exactCallCounts.end(),
           ...
           std::all_of(
               exactCallInputs.begin(), exactCallInputs.end(),
               [](const ExpectedCallInput &input) { return input.count == 1; });
  }

  [[nodiscard]] bool isHirScalarDirectGraph(
      HirFunctionInstanceId id,
      std::unordered_set<HirFunctionInstanceId> &visiting,
      std::unordered_set<HirFunctionInstanceId> &complete) const {
    if (complete.contains(id)) {
      return true;
    }
    if (id == 0 || visiting.contains(id)) {
      return false;
    }
    const HirFunctionInstance *function = hir.findFunctionInstance(id);
    if (function == nullptr || !isHirScalarDirectNodeHeader(*function) ||
        !isHirScalarDirectBody(*function, false)) {
      return false;
    }
    visiting.insert(id);
    for (const HirValue &value : function->body.values) {
      if (value.kind == HirValueKind::Call &&
          (!value.functionTarget ||
           !isHirScalarDirectGraph(*value.functionTarget, visiting,
                                   complete))) {
        visiting.erase(id);
        return false;
      }
  ...
  [[nodiscard]] bool isMirScalarDirectGraph(
      HirFunctionInstanceId id,
      std::unordered_set<HirFunctionInstanceId> &visiting,
      std::unordered_set<HirFunctionInstanceId> &complete) const {
    if (complete.contains(id)) {
    ...
    if (mir == nullptr || id == 0 || visiting.contains(id)) {
      return false;
    }
    const MirFunctionInstance *function = mir->findFunctionInstance(id);
    const HirFunctionInstance *hirFunction = hir.findFunctionInstance(id);
    if (function == nullptr || hirFunction == nullptr ||
        hirFunction->source == nullptr ||
        !isMirScalarDirectNodeHeader(*function, *hirFunction) ||
        !isHirScalarDirectBody(*hirFunction, false) ||
        !isMirScalarDirectBody(*function, *hirFunction)) {
      return false;
    }
    visiting.insert(id);
    for (const MirBlock &block : function->body.blocks) {
      for (const MirInstruction &instruction : block.instructions) {
        if (instruction.kind == MirInstructionKind::Call &&
            (!instruction.functionTarget ||
             !isMirScalarDirectGraph(*instruction.functionTarget, visiting,
                                     complete))) {
          visiting.erase(id);
          return false;
        }
  ...
  [[nodiscard]] const MirFunctionInstance *
  selectedMirScalarDirect(const FunctionDecl &function) const {
    if (mir == nullptr || !function.body()) {
      return nullptr;
    }
    const FunctionInfo *info = semantics.findFunction(function);
    ...
    for (const HirFunctionInstance &candidate : hir.functionInstances()) {
      ...
      if (hirInstance != nullptr) {
        throw std::logic_error(
            "C++ backend found duplicate HIR direct-call instances");
      }
      hirInstance = &candidate;
    }
    if (hirInstance == nullptr) {
      throw std::logic_error(
          "C++ backend is missing an eligible direct-call HIR instance");
    }
    std::unordered_set<HirFunctionInstanceId> hirVisiting;
    std::unordered_set<HirFunctionInstanceId> hirComplete;
    if (hirInstance->source != &function ||
        !isHirScalarDirectNodeHeader(*hirInstance) ||
        !isHirScalarDirectBody(*hirInstance, true) ||
        !isHirScalarDirectGraph(hirInstance->id, hirVisiting, hirComplete)) {
      return nullptr;
    }
    ...
    for (const MirFunctionInstance &candidate : mir->functionInstances()) {
      if (candidate.declaration == info->id) {
        if (selected != nullptr) {
          throw std::logic_error(
              "C++ backend found duplicate MIR direct-call instances");
        }
        selected = &candidate;
      }
    }
    if (selected == nullptr) {
      throw std::logic_error(
          "C++ backend is missing an eligible direct-call MIR instance");
    }
    if (hirInstance == nullptr || hirInstance->source != &function ||
        hirInstance->id != selected->id ||
        hirInstance->declaration != selected->declaration ||
        hirInstance->owner != selected->owner ||
        !hirInstance->typeArguments.empty() ||
        !hirInstance->valueArguments.empty() ||
        ...
        !selected->callableParameters.empty()) {
      throw std::logic_error(
          "eligible MIR scalar direct-call metadata does not match HIR");
    }
    if (!isHirScalarDirectBody(*hirInstance, true)) {
      return nullptr;
    }
    for (const HirFunctionInstanceId id : hirComplete) {
      const MirFunctionInstance *node = mir->findFunctionInstance(id);
      if (node == nullptr) {
        throw std::logic_error(
            "eligible direct-call graph is missing its exact MIR node");
      }
      if (node->mayRaiseDefinedFailure) {
        throw std::logic_error("eligible direct-call graph has a conservative "
                               "MIR defined-failure summary");
      }
    }
    std::unordered_set<HirFunctionInstanceId> visiting;
    std::unordered_set<HirFunctionInstanceId> complete;
    if (!isMirScalarDirectGraph(selected->id, visiting, complete)) {
      throw std::logic_error(
          "failure-free direct-call graph is not closed in verified MIR");
    }
    return selected;
  }
  ...
  hirClassDefaultCleanupClass(const SemanticType &type) const {
    ...
    return selected != nullptr && isHirClassDefaultCleanupClass(*selected)
               ? selected
               : nullptr;
  }

  [[nodiscard]] bool
  isHirClassDefaultCleanupBody(const HirFunctionInstance &function) const {
    if (function.source == nullptr || function.source->body() == nullptr ||
    ...
    const HirBody &body = function.body;
    const StmtList &sourceStatements = function.source->body()->statements();
    if (sourceStatements.size() < 2 || body.bindings.empty() ||
        body.loans.size() != 0 ||
        body.statements.size() != sourceStatements.size() ||
        body.roots.size() != sourceStatements.size() ||
        body.values.size() != sourceStatements.size() ||
        body.fullExpressions.size() != sourceStatements.size() ||
        body.bindings.size() + 1 != sourceStatements.size() ||
        body.dropObligations.size() != body.bindings.size() * 2) {
      return false;
    }

    for (std::size_t index = 0; index < body.bindings.size(); ++index) {
      const auto *variable =
          dynamic_cast<const VariableDecl *>(sourceStatements[index].get());
      const auto *initializer = variable == nullptr
                                    ? nullptr
                                    : dynamic_cast<const DirectInitializer *>(
                                          variable->initializer().get());
      const BindingInfo *semanticBinding =
          variable == nullptr ? nullptr : semantics.findBinding(*variable);
      const ResolvedConstructionInfo *construction =
          initializer == nullptr ? nullptr
                                 : semantics.findConstruction(*initializer);
      const HirStatement &statement = body.statements[index];
      const HirBinding &binding = body.bindings[index];
      const HirValue *value =
          statement.value ? body.findValue(*statement.value) : nullptr;
      const HirClassInstance *classInstance =
          semanticBinding == nullptr
              ? nullptr
              : hirClassDefaultCleanupClass(semanticBinding->type);
      if (variable == nullptr || initializer == nullptr ||
          !initializer->arguments().empty() || variable->isStatic() ||
          variable->isConstexpr() || variable->isRangeBinding() ||
          semanticBinding == nullptr || construction == nullptr ||
          !construction->generatedDefault || construction->constructor != 0 ||
          construction->declaration != nullptr ||
          construction->kind != ConstructorKind::Ordinary ||
          !construction->parameterTypes.empty() ||
          construction->constructedType != semanticBinding->type ||
          classInstance == nullptr || statement.id == 0 ||
          statement.kind != HirStatementKind::Variable ||
          statement.source != variable || !statement.binding ||
          *statement.binding != binding.id || !statement.value ||
          body.roots[index] != statement.id || binding.id == 0 ||
          binding.variable != variable || binding.parameter != nullptr ||
          binding.info.symbol != semanticBinding->symbol ||
          binding.info.type != semanticBinding->type ||
          binding.info.access != semanticBinding->access ||
          !sameMirScalarLeafTraits(binding.info.traits,
                                   semanticBinding->traits) ||
          !binding.dropObligation || value == nullptr ||
          value->kind != HirValueKind::DirectInitializer ||
          value->source != initializer ||
          value->info.type != binding.info.type ||
          !sameMirScalarLeafTraits(value->info.traits, binding.info.traits) ||
          !value->operands.empty() || value->literal || value->operation ||
          value->constructorTarget ||
          value->constructorKind != ConstructorKind::Ordinary ||
          value->unsafeOperation != UnsafeOperationKind::None ||
          !value->definedFailure.empty() || value->ownership ||
          !value->dropObligation) {
        return false;
      }
      const HirDropObligation *valueDrop =
          body.findDropObligation(*value->dropObligation);
      const HirDropObligation *bindingDrop =
          body.findDropObligation(*binding.dropObligation);
      if (valueDrop == nullptr || bindingDrop == nullptr ||
          valueDrop->constructionOrder != body.bindings.size() + index + 1 ||
          valueDrop->kind != HirDropObligationKind::Value ||
          valueDrop->binding != 0 || valueDrop->value != value->id ||
          valueDrop->fullExpression != index + 1 ||
          valueDrop->dropType.type != classInstance->type ||
          ...
          bindingDrop->constructionOrder != index + 1 ||
          bindingDrop->kind != HirDropObligationKind::Binding ||
          bindingDrop->binding != binding.id || bindingDrop->value != 0 ||
          bindingDrop->fullExpression != 0 ||
          bindingDrop->dropType.type != classInstance->type ||
          ...
          bindingDrop->initiallyActive) {
        return false;
      }
      const HirFullExpression &full = body.fullExpressions[index];
      if (full.id != index + 1 || full.statement != statement.id ||
          full.constructorInitializer != 0 ||
          full.roots != std::vector<HirValueId>{value->id} ||
          value->fullExpression != full.id) {
        return false;
      }
    }

    const std::size_t returnIndex = sourceStatements.size() - 1;
    const auto *returnStatement =
        dynamic_cast<const ReturnStmt *>(sourceStatements.back().get());
    const HirStatement &hirReturn = body.statements.back();
    const HirValue *returnValue =
        hirReturn.value ? body.findValue(*hirReturn.value) : nullptr;
    const HirFullExpression &returnFull = body.fullExpressions.back();
    const bool validReturn =
        returnStatement != nullptr && returnStatement->value() != nullptr &&
        hirReturn.kind == HirStatementKind::Return &&
        hirReturn.source == returnStatement && hirReturn.value &&
        body.roots.back() == hirReturn.id && returnValue != nullptr &&
        (returnValue->kind == HirValueKind::Variable ||
         returnValue->kind == HirValueKind::QualifiedName) &&
        returnValue->source == returnStatement->value().get() &&
        returnValue->info.type == function.returnType &&
        returnValue->symbol != 0 && returnValue->operands.empty() &&
        !returnValue->literal && !returnValue->operation &&
        returnValue->unsafeOperation == UnsafeOperationKind::None &&
        returnValue->definedFailure.empty() && !returnValue->dropObligation &&
        returnValue->ownership &&
        returnValue->ownership->kind == OwnershipEventKind::Read &&
        returnValue->ownership->before == OwnershipStateSet::Available &&
        returnValue->ownership->after == OwnershipStateSet::Available &&
        returnValue->ownership->reachable &&
        returnValue->ownership->place.domain == body.placeDomain &&
        returnValue->ownership->place.root == returnValue->symbol &&
        !returnValue->ownership->place.receiver &&
        returnValue->ownership->place.projections.empty() &&
        semantics.findResolvedSymbol(*returnValue->source) ==
            returnValue->symbol &&
        isClassDefaultCleanupGlobal(returnValue->symbol, returnValue->info.type,
                                    false) &&
        returnFull.id == returnIndex + 1 &&
        returnFull.statement == hirReturn.id &&
        returnFull.constructorInitializer == 0 &&
        returnFull.roots == std::vector<HirValueId>{returnValue->id} &&
        returnValue->fullExpression == returnFull.id;
    return validReturn;
  }
  ...
  [[nodiscard]] bool
  isMirClassDefaultCleanupBody(const MirFunctionInstance &lowered,
                               const HirFunctionInstance &source) const {
    const MirBody &body = lowered.body;
    const std::size_t localCount = source.body.bindings.size();
    if (!isHirClassDefaultCleanupBody(source) ||
        body.kind != MirBodyKind::Function ||
        body.placeDomain != source.body.placeDomain || body.entry != 1 ||
        body.returnType != source.returnType || body.blocks.size() != 1 ||
        body.blocks.front().id != body.entry ||
        !body.blocks.front().reachable ||
        body.blocks.front().failureParameter != 0 || !body.loans.empty() ||
        !body.failureRecords.empty() || body.cleanupBoundaries.size() != 1 ||
        body.dropObligations.size() != localCount * 2 ||
        body.fullExpressions.size() != source.body.fullExpressions.size() ||
        body.places.size() != localCount * 2 + 1 ||
        body.values.size() != localCount + 1 ||
        body.blocks.front().instructions.size() != localCount * 4 + 3) {
      return false;
    }

    std::unordered_set<MirPlaceId> exactPlaces;
    std::unordered_set<MirValueId> exactValues;
    std::vector<MirDropObligationId> expectedCleanup;
    expectedCleanup.reserve(localCount);
    const std::vector<MirInstruction> &instructions =
        body.blocks.front().instructions;

    for (std::size_t index = 0; index < localCount; ++index) {
      const HirBinding &hirBinding = source.body.bindings[index];
      const HirStatement &hirStatement = source.body.statements[index];
      const HirValue *hirValue =
          hirStatement.value ? source.body.findValue(*hirStatement.value)
                             : nullptr;
      const HirDropObligation *hirValueDrop =
          hirValue == nullptr || !hirValue->dropObligation
              ? nullptr
              : source.body.findDropObligation(*hirValue->dropObligation);
      const HirDropObligation *hirBindingDrop =
          !hirBinding.dropObligation
              ? nullptr
              : source.body.findDropObligation(*hirBinding.dropObligation);
      const MirDropObligation *valueDrop =
          hirValueDrop == nullptr
              ? nullptr
              : mirClassDefaultCleanupDrop(body, hirValueDrop->id);
      const MirDropObligation *bindingDrop =
          hirBindingDrop == nullptr
              ? nullptr
              : mirClassDefaultCleanupDrop(body, hirBindingDrop->id);
      const MirPlace *valuePlace =
          valueDrop == nullptr ? nullptr : body.findPlace(valueDrop->place);
      const MirPlace *bindingPlace =
          bindingDrop == nullptr ? nullptr : body.findPlace(bindingDrop->place);
      const HirClassInstance *hirClass =
          hirClassDefaultCleanupClass(hirBinding.info.type);
      const MirClassInstance *mirClass =
          hirClass == nullptr || mir == nullptr
              ? nullptr
              : mir->findClassInstance(hirClass->id);
      if (hirValue == nullptr || hirValueDrop == nullptr ||
          hirBindingDrop == nullptr || valueDrop == nullptr ||
          bindingDrop == nullptr || valuePlace == nullptr ||
          bindingPlace == nullptr || hirClass == nullptr ||
          mirClass == nullptr ||
          !isMirClassDefaultCleanupClass(*mirClass, *hirClass) ||
          valueDrop->constructionOrder != index * 2 + 1 ||
          valueDrop->kind != MirDropObligationKind::Value ||
          valueDrop->binding != 0 || valueDrop->value != hirValue->id ||
          valueDrop->hirFullExpression != hirValueDrop->fullExpression ||
          valueDrop->fullExpression != hirValueDrop->fullExpression ||
          valueDrop->dropType.type != hirValueDrop->dropType.type ||
          valueDrop->dropType.classInstance !=
              hirValueDrop->dropType.classInstance ||
          valueDrop->dropType.destructor != hirValueDrop->dropType.destructor ||
          !valueDrop->dropType.requiresActiveCleanup ||
          valueDrop->initiallyActive ||
          bindingDrop->constructionOrder != index * 2 + 2 ||
          bindingDrop->kind != MirDropObligationKind::Binding ||
          bindingDrop->binding != hirBinding.id || bindingDrop->value != 0 ||
          bindingDrop->hirFullExpression != 0 ||
          bindingDrop->fullExpression != 0 ||
          bindingDrop->dropType.type != hirBindingDrop->dropType.type ||
          bindingDrop->dropType.classInstance !=
              hirBindingDrop->dropType.classInstance ||
          bindingDrop->dropType.destructor !=
              hirBindingDrop->dropType.destructor ||
          !bindingDrop->dropType.requiresActiveCleanup ||
          bindingDrop->initiallyActive ||
          valuePlace->root != MirPlaceRootKind::Value ||
          valuePlace->binding != 0 || valuePlace->symbol != 0 ||
          valuePlace->capture != 0 || valuePlace->temporary != 0 ||
          valuePlace->value == 0 || valuePlace->loan != 0 ||
          !valuePlace->projections.empty() || valuePlace->key ||
          valuePlace->initiallyAvailable ||
          valuePlace->sourceValue != hirValue->id ||
          valuePlace->type != hirValue->info.type ||
          !sameMirScalarLeafTraits(valuePlace->traits, hirValue->info.traits) ||
          bindingPlace->root != MirPlaceRootKind::Binding ||
          bindingPlace->binding != hirBinding.id ||
          bindingPlace->symbol != hirBinding.info.symbol ||
          bindingPlace->capture != 0 || bindingPlace->temporary != 0 ||
          bindingPlace->value != 0 || bindingPlace->loan != 0 ||
          !bindingPlace->projections.empty() || !bindingPlace->key ||
          bindingPlace->initiallyAvailable || bindingPlace->sourceValue != 0 ||
          bindingPlace->type != hirBinding.info.type ||
          bindingPlace->access != hirBinding.info.access ||
          !sameMirScalarLeafTraits(bindingPlace->traits,
                                   hirBinding.info.traits) ||
          bindingPlace->key->domain != body.placeDomain ||
          bindingPlace->key->root != hirBinding.info.symbol ||
          bindingPlace->key->receiver ||
          !bindingPlace->key->projections.empty() ||
          !exactPlaces.insert(valuePlace->id).second ||
          !exactPlaces.insert(bindingPlace->id).second) {
        return false;
      }

      const MirInstruction &construct = instructions[index * 3];
      const MirInstruction &initialize = instructions[index * 3 + 1];
      const MirInstruction &boundary = instructions[index * 3 + 2];
      ...
      if (construct.kind != MirInstructionKind::Construct ||
          construct.hirValue != hirValue->id || construct.hirStatement != 0 ||
          !construct.result || construct.destination ||
          !construct.operands.empty() ||
          construct.operation != MirOperation::None || construct.literal ||
          construct.ownership || construct.fullExpressionEnd != 0 ||
          construct.cleanupBoundaryEnd != 0 ||
          construct.lifecycle.size() != 1 ||
          !hasOnlyMirClassDefaultCleanupMetadata(construct) ||
          !isMirClassDefaultCleanupLifecycle(construct.lifecycle.front(),
                                             MirLifecycleEventKind::Initialize,
                                             0, valueDrop->id) ||
          construct.info.type != hirValue->info.type ||
          !sameMirScalarLeafTraits(construct.info.traits,
                                   hirValue->info.traits) ||
          result == nullptr || result->definition != construct.id ||
          result->definitionBlock != body.entry ||
          result->sourceValue != hirValue->id ||
          !sameMirScalarLeafInfo(result->info, hirValue->info) ||
          valuePlace->value != result->id ||
          !exactValues.insert(result->id).second ||
          initialize.kind != MirInstructionKind::Initialize ||
          initialize.hirValue != 0 ||
          initialize.hirStatement != hirStatement.id || initialize.result ||
          initialize.destination != bindingPlace->id ||
          initialize.operands.size() != 1 ||
          initialize.operands.front().kind != MirOperandKind::Value ||
          initialize.operands.front().value != result->id ||
          initialize.operands.front().type != result->info.type ||
          initialize.operation != MirOperation::None || initialize.literal ||
          initialize.ownership || initialize.fullExpressionEnd != 0 ||
          initialize.cleanupBoundaryEnd != 0 ||
          initialize.lifecycle.size() != 1 ||
          !hasOnlyMirClassDefaultCleanupMetadata(initialize) ||
          !isMirClassDefaultCleanupLifecycle(initialize.lifecycle.front(),
                                             MirLifecycleEventKind::Reparent,
                                             valueDrop->id, bindingDrop->id) ||
          initialize.info.type != hirBinding.info.type ||
          !sameMirScalarLeafTraits(initialize.info.traits,
                                   hirBinding.info.traits) ||
          boundary.kind != MirInstructionKind::Lifecycle ||
          boundary.hirValue != hirValue->id ||
          boundary.hirStatement != hirStatement.id || boundary.result ||
          boundary.destination || !boundary.operands.empty() ||
          boundary.operation != MirOperation::None || boundary.literal ||
          boundary.ownership || !boundary.lifecycle.empty() ||
          !hasOnlyMirClassDefaultCleanupMetadata(boundary) ||
          boundary.fullExpressionEnd != index + 1 ||
          boundary.cleanupBoundaryEnd != 0) {
        return false;
      }
      expectedCleanup.push_back(bindingDrop->id);
    }

    const std::size_t returnBase = localCount * 3;
    const HirStatement &hirReturn = source.body.statements.back();
    const HirValue *hirReturnValue =
        hirReturn.value ? source.body.findValue(*hirReturn.value) : nullptr;
    const MirInstruction &load = instructions[returnBase];
    const MirInstruction &returnBoundary = instructions[returnBase + 1];
    const MirPlace *globalPlace =
        load.operands.size() == 1 ? body.findPlace(load.operands.front().place)
                                  : nullptr;
    const MirValue *returnValue =
        load.result ? body.findValue(*load.result) : nullptr;
    if (hirReturnValue == nullptr || load.kind != MirInstructionKind::Load ||
        load.hirValue != hirReturnValue->id || load.hirStatement != 0 ||
        !load.result || load.destination || load.operands.size() != 1 ||
        ...
        !load.lifecycle.empty() ||
        !hasOnlyMirClassDefaultCleanupMetadata(load) ||
        returnValue == nullptr || returnValue->definition != load.id ||
        returnValue->definitionBlock != body.entry ||
        returnValue->sourceValue != hirReturnValue->id ||
        !sameMirScalarLeafInfo(returnValue->info, hirReturnValue->info) ||
        !exactValues.insert(returnValue->id).second || globalPlace == nullptr ||
        globalPlace->root != MirPlaceRootKind::Symbol ||
        globalPlace->binding != 0 ||
        globalPlace->symbol != hirReturnValue->symbol ||
        globalPlace->capture != 0 || globalPlace->temporary != 0 ||
        globalPlace->value != 0 || globalPlace->loan != 0 ||
        !globalPlace->projections.empty() || !globalPlace->key ||
        globalPlace->initiallyAvailable ||
        globalPlace->sourceValue != hirReturnValue->id ||
        globalPlace->type != hirReturnValue->info.type ||
        !sameMirScalarLeafTraits(globalPlace->traits,
                                 hirReturnValue->info.traits) ||
        globalPlace->key->domain != body.placeDomain ||
        globalPlace->key->root != hirReturnValue->symbol ||
        globalPlace->key->receiver || !globalPlace->key->projections.empty() ||
        !isMirScalarCfgReadEvent(load.ownership, body, globalPlace) ||
        !sameMirClassDefaultCleanupOwnership(load.ownership,
                                             hirReturnValue->ownership) ||
        !isClassDefaultCleanupGlobal(globalPlace->symbol, globalPlace->type,
                                     false) ||
        !exactPlaces.insert(globalPlace->id).second ||
        returnBoundary.kind != MirInstructionKind::Lifecycle ||
        returnBoundary.hirValue != hirReturnValue->id ||
        returnBoundary.hirStatement != hirReturn.id || returnBoundary.result ||
        returnBoundary.destination || !returnBoundary.operands.empty() ||
        returnBoundary.operation != MirOperation::None ||
        returnBoundary.literal || !returnBoundary.lifecycle.empty() ||
        !hasOnlyMirClassDefaultCleanupMetadata(returnBoundary) ||
        returnBoundary.fullExpressionEnd != localCount + 1 ||
        returnBoundary.cleanupBoundaryEnd != 0 ||
        !isMirScalarCfgReadEvent(returnBoundary.ownership, body, globalPlace) ||
        !sameMirClassDefaultCleanupOwnership(returnBoundary.ownership,
                                             hirReturnValue->ownership)) {
      return false;
    }
    ...
    for (std::size_t reverse = localCount; reverse > 0; --reverse) {
      const HirBinding &hirBinding = source.body.bindings[reverse - 1];
      const HirDropObligation *hirDrop =
          hirBinding.dropObligation
              ? source.body.findDropObligation(*hirBinding.dropObligation)
              : nullptr;
      const MirDropObligation *drop =
          hirDrop == nullptr ? nullptr
                             : mirClassDefaultCleanupDrop(body, hirDrop->id);
      const MirInstruction &instruction = instructions[instructionIndex++];
      if (drop == nullptr || instruction.kind != MirInstructionKind::Drop ||
          instruction.hirValue != 0 || instruction.hirStatement != 0 ||
          instruction.result || instruction.destination != drop->place ||
          !instruction.operands.empty() ||
          instruction.operation != MirOperation::None || instruction.literal ||
          instruction.ownership || instruction.fullExpressionEnd != 0 ||
          instruction.cleanupBoundaryEnd != 0 ||
          instruction.lifecycle.size() != 1 ||
          !hasOnlyMirClassDefaultCleanupMetadata(instruction) ||
          !isMirClassDefaultCleanupLifecycle(instruction.lifecycle.front(),
                                             MirLifecycleEventKind::Drop,
                                             drop->id, 0) ||
          instruction.info.type != hirBinding.info.type ||
          !sameMirScalarLeafTraits(instruction.info.traits,
                                   hirBinding.info.traits)) {
        return false;
      }
    }

    const MirInstruction &cleanupMarker = instructions[instructionIndex];
    std::reverse(expectedCleanup.begin(), expectedCleanup.end());
    const MirCleanupBoundary &cleanup = body.cleanupBoundaries.front();
    if (cleanup.id != 1 || cleanup.kind != MirCleanupBoundaryKind::Normal ||
        cleanup.obligations != expectedCleanup ||
        cleanupMarker.kind != MirInstructionKind::Lifecycle ||
        cleanupMarker.hirValue != 0 || cleanupMarker.hirStatement != 0 ||
        cleanupMarker.result || cleanupMarker.destination ||
        !cleanupMarker.operands.empty() ||
        cleanupMarker.operation != MirOperation::None ||
        cleanupMarker.literal || cleanupMarker.ownership ||
        !cleanupMarker.lifecycle.empty() ||
        !hasOnlyMirClassDefaultCleanupMetadata(cleanupMarker) ||
        cleanupMarker.fullExpressionEnd != 0 ||
        cleanupMarker.cleanupBoundaryEnd != cleanup.id ||
        exactPlaces.size() != body.places.size() ||
        exactValues.size() != body.values.size()) {
      return false;
    }
    ...
    const MirTerminator &terminator = body.blocks.front().terminator;
    return terminator.kind == MirTerminatorKind::Return && terminator.value &&
           terminator.value->kind == MirOperandKind::Value &&
           terminator.value->value == returnValue->id &&
           terminator.value->type == source.returnType &&
           terminator.hirValue == 0 &&
           terminator.hirStatement == hirReturn.id && !terminator.returnLoan &&
           terminator.invokeInstruction == 0 && terminator.failureRecord == 0 &&
           terminator.target == 0 && terminator.elseTarget == 0 &&
           terminator.switchTargets.empty() &&
           terminator.successLifecycle.empty();
  ...
  [[nodiscard]] const MirFunctionInstance *
  selectedMirClassDefaultCleanup(const FunctionDecl &function) const {
    ...
    for (const HirFunctionInstance &candidate : hir.functionInstances()) {
      ...
      if (hirInstance != nullptr) {
        throw std::logic_error(
            "C++ backend found duplicate HIR class-cleanup instances");
      }
      hirInstance = &candidate;
    }
    if (hirInstance == nullptr || hirInstance->source != &function ||
        !isHirClassDefaultCleanupBody(*hirInstance)) {
      return nullptr;
    }
    ...
    for (const MirFunctionInstance &candidate : mir->functionInstances()) {
      ...
      if (selected != nullptr) {
        throw std::logic_error(
            "C++ backend found duplicate MIR class-cleanup instances");
      }
      selected = &candidate;
    }
    if (selected == nullptr || hirInstance->id != selected->id ||
        hirInstance->declaration != selected->declaration ||
        hirInstance->owner != selected->owner ||
        !hirInstance->typeArguments.empty() ||
        !hirInstance->valueArguments.empty() ||
        ...
        hirInstance->overrideMethod != selected->overrideMethod ||
        hirInstance->virtualRoots != selected->virtualRoots ||
        !hirInstance->callableParameters.empty() || selected->owner ||
        selected->staticMember ||
        selected->receiverMutability != ReceiverMutability::ReadOnly ||
        selected->overloadedOperator || selected->constexprFunction ||
        selected->definitionKind !=
      ...
      throw std::logic_error(
          "eligible MIR class-cleanup metadata does not match HIR");
    }
    for (const HirBinding &binding : hirInstance->body.bindings) {
      const HirClassInstance *classInstance =
          hirClassDefaultCleanupClass(binding.info.type);
      const HirDestructorInstance *destructor =
          classInstance == nullptr || !classInstance->destructor
              ? nullptr
              : hir.findDestructorInstance(*classInstance->destructor);
      if (destructor == nullptr || destructor->source == nullptr ||
          selectedMirClassDefaultCleanupDestructor(*destructor->source) ==
              nullptr) {
        throw std::logic_error(
            "eligible class-cleanup body lacks its verified MIR destructor");
      }
    }
    if (!isMirClassDefaultCleanupBody(*selected, *hirInstance)) {
      throw std::logic_error(
          "eligible class-default-cleanup source body is not represented by "
          "the closed verified MIR family");
    }
    return selected;
  }
  ...
  *result = static_cast<Target>(value);
  ...
  void emitMirScalarDirectTarget(const MirInstruction &instruction) {
    ...
    if (target == nullptr || hirTarget == nullptr || info == nullptr ||
        info->id != target->declaration ||
        info->declaration != hirTarget->source ||
        !isMirScalarDirectNodeHeader(*target, *hirTarget) ||
        target->mayRaiseDefinedFailure ||
        target->definitionKind != MirFunctionInstance::DefinitionKind::Source) {
      throw std::logic_error(
  ...
  void emitMirScalarDirectInstruction(const MirInstruction &instruction) {
    if (instruction.kind != MirInstructionKind::CallInput &&
    ...
    emitMirScalarDirectTarget(instruction);
    output << '(';
    for (std::size_t index = 0; index < instruction.operands.size(); ++index) {
      if (index != 0) {
        output << ", ";
  ...
  [[nodiscard]] static MirPlaceId
  mirClassDefaultCleanupDestination(const MirFunctionInstance &function,
  ...
  void emitMirClassDefaultCleanupBody(const MirFunctionInstance &function) {
    output << "{\n";
    ++indentation;
    writeIndent();
    output << "// GTI verified-MIR body: class-default-cleanup-v1 "
    ...
    for (const MirPlace &place : function.body.places) {
      ...
      output << "::gti_internal::backend::mir_lifetime_slot<";
      emitSemanticType(place.type);
      output << "> __gti_mir_cleanup_p_" << place.id << ";\n";
    }
    for (const MirValue &value : function.body.values) {
    ...
    for (const MirInstruction &instruction :
      ...
      case MirInstructionKind::Construct: {
        const MirPlaceId destination =
            mirClassDefaultCleanupDestination(function, instruction);
        if (destination == 0) {
          throw std::logic_error(
              "verified MIR class construction lost its reparent target");
        }
        output << "__gti_mir_cleanup_p_" << destination << ".construct();\n";
        break;
      }
      case MirInstructionKind::Initialize:
      ...
      case MirInstructionKind::Drop:
        output << "__gti_mir_cleanup_p_" << *instruction.destination
               << ".destroy();\n";
        break;
      case MirInstructionKind::Lifecycle:
        if (instruction.fullExpressionEnd != 0) {
      ...
      default:
        throw std::logic_error(
            "verified MIR class cleanup instruction is unsupported");
      }
    }
    writeIndent();
    output << "return __gti_mir_cleanup_v_"
