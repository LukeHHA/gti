...
| ID | Current restriction or gap | Class | 1.0 disposition | Owner and reconsideration evidence |
| --- | --- | --- | --- | --- |
| `R-LAYOUT-QUERIES` | The compiler owns deterministic scalar data-layout facts, but source still has no `sizeof`/`alignof`, and ordinary class/virtual/native layout is not a GTI fact. | choice | **close-v1** | `S-LAYOUT-01` is complete; ready `S-LAYOUT-02` adds the bounded source surface for primitives, raw pointers, fixed arrays, and explicitly layout-stable records only. Values are frontend constants checked against target probes. |
