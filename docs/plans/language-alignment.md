...
| `R-MEMORY-MODEL` | ADR 008 and Execution §4.9 define safe data-race freedom, transfer/share facts, an explicit concurrent profile, SC atomic and owned automatic-join first boundaries, happens-before, globals, worker failure, and native entry. C-TYPE-01 now implements structural facts, public concepts, and nominal policy while the executable profile remains single-threaded. | choice | **keep-v1** | `D-MEM-02`, `I-CAP-01`, and `C-TYPE-01` are complete. `C-GLOBAL-01` implements the remaining pre-1.0 global/static policy without exposing public concurrency. Any incompatible memory-model change requires the compatibility mechanism. |
...
| `R-GLOBAL-OWNERSHIP` | Reference, borrowed-state, and unique-owner globals are rejected. Cleanup-owning globals are normatively unavailable but declared-cleanup value types currently expose a semantic trait hole; mutable ordinary globals exist only in the current single-thread profile. ADR 008 requires concurrent-profile globals/statics to be immutable and share-capable, with synchronized mutation behind an approved wrapper. | proof/choice | **bounded-v1** | `M-LIFE-01` closes the recursive cleanup-owning global/static hole. `C-GLOBAL-01` implements the adopted profile-specific rule without changing current single-threaded globals. Broader process-lifetime ownership requires a new row after global initialization, shutdown, failure, and foreign-thread participation are explicit. |
