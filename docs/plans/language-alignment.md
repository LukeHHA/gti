...
| `R-NATIVE-RECORDS` | **Closed in the bounded native-record release:** `[[c_abi]] struct` opts a passive, non-owning fixed-width/nested-record/one-level-pointer field family into compiler-owned source-order layout and by-value or one-level-pointer `extern "C"` passage. Ordinary records, arrays, enums, bool/char, owners, references, and cleanup-owning values remain excluded. | choice | **bounded-first** | `S-ABI-01/02` and ADR 013 are complete. A native C oracle proves size, alignment, offsets, nested records, by-value return/passage, and pointer mutation at O0/O3 and C++20/C++23. Reopened from **closed** for the fixed-array field family only: ADR 013 excluded `T[N]` fields solely until their backend representation was defined directly rather than inherited from `std::array`, and `S-FFI-02` family F1 supplies that against the GLFW acceptance client. C array layout is unambiguous, so no layout proof is surrendered. `bool`, `char`, enums, owners, references, and cleanup-owning values stay excluded. |
| `R-C-STRINGS` | No boundary type carries C's NUL-terminated string contract. A C `const char*` must be spelled `const uint8_t*`, which is layout-correct but discards the terminator invariant and forces a hand conversion from GTI strings at every call site. | choice | **systems-ready** | `S-FFI-02` family F2 defines one boundary type with the NUL-terminated contract, conversion from GTI strings without a call-site copy, and a bounded read-back view. `char` itself stays excluded from `[[c_abi]]` records under `R-NATIVE-RECORDS`; the gap is the missing contract, not the missing scalar. Roughly 30 GLFW entry points depend on it. |
