...
| `R-C-ABI-FAMILIES` | **Partially closed:** `[[c_opaque]] struct Name;` supplies nominal pointer-only handle identity, fixed arrays are admitted in passive native records, `c_string` carries the NUL-terminated boundary, and `[[c_array(count)]]` supplies the bounded returned pointer-plus-count family. General pointer nesting, annotated ownership transfer, array parameters, casts, varargs, native variables, alternate calling conventions, and C++ linkage remain absent. | choice | **bounded-first** | `S-ABI-03` generates one checked C17/C++20/C++23 adapter header; completed `S-FFI-02` sub-slices cover opaque identity and GLFW's fixed-array/string/returned-array families without adding object ABI, inferred ownership, or exception passage. Callback function types are the next measured blocker. Varargs, unions, bit-fields, packing, and general C++ linkage remain later breadth. |
