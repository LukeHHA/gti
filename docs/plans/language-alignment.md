...
| ID | Current restriction or gap | Class | 1.0 disposition | Owner and reconsideration evidence |
| --- | --- | --- | --- | --- |
| `R-PLACE-STATE` | Precise places cover roots, fields, and bounded checked dereferences, but constant/dynamic indexed partial movement, complete definite initialization, and general reinitialization are incomplete. M-OWN-01 now fixes the value-key, exhaustive relation, finite state transfer, phase authority, and invalidation contract without claiming implementation. | proof | **close-v1** | `M-OWN-01` is done; `M-OWN-02` implements the constant-indexed directly owned slice. Semantic and MIR state must agree at branches, loop backedges, return, and drop without backend repair. |
