...
| `R-CALLABLES` | Lambdas use explicit typed parameters/results and immutable copy snapshots. Confined generic calls support exact `void`, `bool`, and context-supplied owned value results. Capture defaults, reference/init/move capture, mutable closures, `auto` result inference, borrowed/callable-valued results, callable references, field/global storage, and general escape are rejected. | proof | **systems-ready** | D-CALL-01 defines exact concrete identity, signature, read/mut/once capability, lifecycle, and confined/owned escape in the accepted [callable contract](callable-ownership-and-escape.md). L-CALL-01 has completed the confined boundary and value-result slices; invocation classification, owned callable transport, and move capture remain. General type erasure, reference capture, recursive closures, and unconstrained escape remain later breadth. |
