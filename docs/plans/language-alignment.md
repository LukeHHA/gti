...
3. The GTI-owned target data-layout contract and bounded `sizeof`/`alignof`
   are implemented. Defined wrapping/saturating integer operations and
   IEEE-754 binary64 remain required for systems readiness.
...
| `R-INTEGER-MODES` | Built-in integer arithmetic remains checked by default and there is no explicit wrapping, saturating, or checked-result operation family. | library | **systems-ready** | `L-NUM-01`; exhaustive domains, constexpr/runtime/O0/O3 parity, and optimizer evidence must ship without making `unsafe` disable checks. |
