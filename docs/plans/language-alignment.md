...
| `R-BUILD-CONFIG` | Compile-time conditionals select on the target triple only (`target.os`, `target.vendor`, `target.arch`), with a single comparison and no boolean operators. Build configuration — debug instrumentation, optional subsystems, downstream feature selection — cannot be expressed, so a unit that must vary by configuration ships as separate source or `#error`s, as `stdlib/std/tcp.gti` does on Windows. | choice | **design-first** | `docs/plans/conditional-compilation.md` proposes a valueless `#define`/`#ifdef` flag axis plus boolean conditions, deliberately excluding substitution and function-like macros so that every branch stays parseable GTI. Sequencing waits on the `M-BACK-02` cutover. |
