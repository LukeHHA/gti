...
| --- | --- | --- | --- | --- |
| `R-EXEC-ORDER` | ADR 010 and Execution Section 4.2 define strict left-to-right operands/arguments, target-first assignment, initialization, full-expression cleanup, and program-wide initialization. Normal-exit obligations are authoritative, but the complete executable schedule is not, so a transient borrow and overlapping mutation remain conservatively rejected in one call regardless of written order. | lowering | **systems-ready** | `D-EXEC-01` and `M-LIFE-01` are done. `M-EXEC-01` provides ordered MIR; `M-BACK-01/02` migrate affected production families before each conservative restriction is narrowed. |
| `R-TEMP-DROP` | Typed lexical and materializing-value obligations, reverse normal cleanup, ownership transfer, and path-conditional availability are authoritative in HIR/MIR. Direct self-contained class-value construction/call results now support verified ephemeral read/mutable method receivers with exactly-once full-expression cleanup. Ordered compound materialization, partial-construction rollback, and wider checked-failure cleanup remain incomplete. | lowering | **systems-ready** | `M-LIFE-01` and `M-OWN-02` are done. The direct temporary-receiver `M-EXEC-01`/backend slice is complete; remaining `M-EXEC-01` and `M-FAIL-01` families provide broader order/rollback/failure facts and matching production authority. |
| `R-FAIL` | Execution §4.10 now defines categories, artifact-qualified sites, status/reporting, cleanup, observation, embedding, allocation, and worker containment, but the emitter still aborts without those semantics and native expected observers still escape them. | lowering | **systems-ready** | `D-FAIL-01`, `I-CAP-01`, and the normal-exit lifecycle substrate in `M-LIFE-01` are done; the relevant `M-EXEC-01` slices and co-delivered `M-FAIL-01`/`Q-FAIL-01` implement the remaining IR/runtime substrate; `M-BACK-02` then migrates every closed call-graph family and removes native helper behavior. |
...
| `R-BACKEND-AUTHORITY` | `CppBackend` now emits the exact `scalar-leaf-v1`, `scalar-cfg-v1`, `scalar-direct-call-v1`, `class-default-cleanup-v1`, and `owned-lifecycle-call-v1` families wholly from verified optimized MIR, but all other complete bodies still execute from AST/semantic/HIR compatibility data; some accepted evaluation/lifetime behavior is also not yet complete in MIR. | lowering | **fix-now** | `M-BACK-01` and the M-BACK-02 failure-free phases through owned lifecycle are done. The failure-capable M-BACK-02 closure is active; the whole `M-EXEC-01` row is not a prerequisite. Consolidated later M-BACK-02 phases co-deliver missing per-family M-EXEC/M-FAIL facts, migrate failure-capable closures, and finally remove AST/HIR body execution. Systems readiness requires every accepted observable order, failure, lifetime, and cleanup rule to be represented independently of C++. |
...
| `R-DIRECT-CONSTRUCTION` | `Type value{args}` is exact direct construction, not aggregate/list conversion, initializer-list preference, narrowing policy, or CTAD. Contextual braces inside a call are separately defined by ADR 015 as one exact fixed-array value, never a class-list conversion. | safety/simplicity | **durable-rule** | Current static semantics and ADR 015. `std::initializer_list`, list-preferred overloads, and CTAD remain excluded unless a new decision replaces the exact-array model. |
...
| ID | Current restriction or gap | Class | Readiness role | Owner and reconsideration evidence |
| --- | --- | --- | --- | --- |
| `R-PLACE-STATE` | Precise places cover roots, fields, bounded checked dereferences, and directly owned fixed-array elements selected by an in-range constant. Semantic state and MIR verification agree across moves, restoration, branches, loop backedges, and supported normal-exit active-drop obligations. Dynamic-index movement, arbitrary provenance, lifetime epochs, and failure rollback remain unavailable. | proof | **bounded-first** | `M-OWN-01`, `M-OWN-02`, and `M-LIFE-01` are done. Any dynamic-index or broader provenance precision needs a separate proof row; failure cleanup remains with `M-FAIL-01`. |
...
| `R-MOVE-PLACES` | Explicit movement supports locals, by-value parameters, writable named fields, checked owner projections, and in-range constant elements of directly owned fixed arrays. Globals, already-created capture fields, borrowed fields, temporaries, and dynamic indexed places are rejected. The closure initializer `[target = std::move(source)]` is a separate implemented transfer from an enclosing local/parameter into a new immutable environment field. | proof | **bounded-first** | `M-OWN-02` and the local move-capture slice of `L-CALL-01` are complete. Any additional form needs a named client and state owner. |
| `R-GLOBAL-OWNERSHIP` | Reference, borrowed-state, unique-owner, and recursively cleanup-owning globals/statics are rejected. Mutable ordinary globals remain available only under default single-threaded selection; the implemented concurrent profile requires immutable share-capable globals/statics, with synchronized mutation behind a future approved wrapper. | proof/choice | **bounded-first** | `C-GLOBAL-01` and `M-LIFE-01` are complete: declared cleanup is followed through aliases, arrays, bases, fields, captures, and concrete generic substitutions. Broader process-lifetime ownership requires a new row after global initialization, shutdown, failure, and foreign-thread participation are explicit. |
...
| ID | Current restriction or gap | Class | Readiness role | Owner and reconsideration evidence |
| --- | --- | --- | --- | --- |
| `R-CONCEPTS` | User-defined multi-parameter conjunction concepts and bounded trailing `requires` conjunctions are implemented. Exact qualified class/struct specializations are also implemented with canonical keys and package coherence. The current structural facts cover input-iterator dereference/increment, exact iterator/sentinel inequality, and exact accumulation referents. Disjunction, negation, general requires-expressions, arbitrary expression requirements, associated types, partial or constraint-selected specialization, subsumption, and constraint-based overload ranking remain absent; callable/complete-range/hash capabilities are incomplete. | safety/simplicity | **bounded-first** | [ADR 009](../decisions/009-bounded-requires-contracts.md) keeps requirements validity-only and preserves exact overloads; [ADR 020](../decisions/020-exact-qualified-class-specializations.md) admits only exact semantic selection. D-CALL-01's accepted [callable contract](callable-ownership-and-escape.md) defines exact signatures plus read/mut/once capability without adding ranking. `L-CALL-01`, `L-RANGE-04`, and `L-CONT-02` supply the next client-driven capabilities. General expression requirements, partial specialization, subsumption, and ranking remain later breadth unless a client cannot be expressed by bounded exact relations. |
| `R-VALUE-GENERICS` | Class/struct value generics are immutable `uint64_t` values with literal/parameter arguments. Functions, methods, and constructors additionally admit one inferred-only family: the value parameter must be the complete extent of a by-value fixed-array parameter, supplied by a named array or contextual brace count. Explicit callable value arguments, other value types, expressions, defaults, and packs remain rejected. | proof | **bounded-first** | ADR 015 and `L-INIT-01` close the exact array-extent client with concrete HIR identity. `L-CONST-01` owns broader compile-time execution/defaults; unrestricted metaprogramming and partial specialization are not reconsideration evidence. |
| `R-CONSTEXPR` | Constant evaluation is scalar and bounded. Generic constexpr function/value execution, aggregate values, references, allocation, arrays as values, and runtime/native calls remain rejected. Exact-`bool` static assertions are implemented, including deferred evaluation for canonical concrete generic class, function, method, and constructor instances. | proof | **bounded-first** | `L-CONST-01` requires a concrete library or domain client and constexpr/runtime parity for broader execution. The implemented [static-assertion design](static-assertions.md) reuses the same evaluator and source diagnostics and erases before HIR. Native C++ evaluation is never language authority. |
| `R-CALLABLES` | Lambdas use explicit typed parameters/results and immutable fields initialized by bare copy capture or exact `[target = std::move(source)]` owned capture. Confined generic calls support exact `void`, `bool`, and context-supplied non-reference value results without tracked borrowed state or lambda identity. Immutable parameters require repeatable read-callable targets; `mut` parameters accept read-callable or mut-callable targets; `std::move(parameter)()` requires once-callable use and may select a reusable target or nominal `operator() &&`. A free function's exact direct generic parameter may be explicitly moved through the same-type result or into the exact sole field of a generic owner. Capture defaults, reference/general init capture, mutable closures, `auto` result inference, borrowed/lambda-valued results, callable references, arbitrary field/global storage, and general escape are rejected. | proof | **systems-ready** | D-CALL-01 defines exact concrete identity, signature, read/mut/once capability, lifecycle, and confined/owned escape in the accepted [callable contract](callable-ownership-and-escape.md). L-CALL-01 has completed the confined boundary, value-result, repeatable invocation, consuming-cardinality, local callable-environment lifecycle/move capture, exact generic return, and bounded one-field owner slices. General type erasure, reference capture, recursive closures, broader owner extraction, and unconstrained escape remain later breadth. |
| `R-FUNCTION-VALUES` | Function names must normally be called. The bounded exception is exact contextual conversion of an eligible namespace free function to a named native callback alias; ordinary function items and general function-pointer values are not first-class. | lowering | **systems-ready** | `S-CALL-01` implements the demonstrated non-capturing same-thread C callback boundary with semantic selection and verified HIR/MIR adapter identity. Other callable clients can extend the accepted [callable contract](callable-ownership-and-escape.md) without introducing C++ function-pointer inference. |
| `R-SUM-TYPES` | **Partially closed:** a non-generic scoped enum may declare passive payload cases, construct them exactly, and destructure copied immutable payloads in an exhaustive `switch`. Cleanup-owning, borrowed, moved, generic, and stable-layout payload families remain absent; `expected` remains a dedicated result form. | choice | **systems-ready** | `L-SUM-01`; the first passive family is complete. Ownership-aware matching, partial initialization/drop, generic payloads, and deterministic layout remain bounded follow-on work for the compiler-AST/protocol workload. Native unions are deliberately separate untagged storage. |
...
| ID | Current restriction or gap | Class | Readiness role | Owner and reconsideration evidence |
| --- | --- | --- | --- | --- |
| `R-LAYOUT-QUERIES` | Reserved, type-only `sizeof(type)`/`alignof(type)` produce exact `uint64_t` frontend constants for primitives, one-level raw pointers, integral scoped enums, transparent aliases, recursive positive concrete fixed arrays, valid passive unions, and valid passive `[[c_abi]]` records. Pointer layout is independent of pointee layout. ABI alignment is queryable; preferred alignment is not. Zero/symbolic extents, overflow, direct symbolic types, references, ordinary nominal aggregates, payload enums, and every other backend-dependent category are rejected before lowering. | choice | **closed** | `S-LAYOUT-01/02`, `S-ABI-01/02`, and the passive-union slice of `L-SUM-01` are complete. Synthetic supported-target checks plus native and installed-library probes cover the GTI-owned facts. Expression operands, direct query expressions in array-extent grammar, payload representation, and ordinary class layout remain outside the bounded contract. |
| `R-LAYOUT-CONTROL` | `alignas`, packed records, bit-fields, and stable layout for ordinary classes remain absent. GTI now has a bounded passive native union whose every member access requires `unsafe`; it is not a C ABI declaration and has no compiler-tracked active member. | safety/simplicity | **bounded-first** | `S-ABI-01/02` completed layout-stable passive native records. The first union family supplies explicit overlapping storage while retaining target-owned size/alignment facts and trivial-copy restrictions. General alignment control, MMIO, packing, bit-fields, and C ABI unions remain later breadth. |
| `R-NATIVE-RECORDS` | **Closed in the bounded native-record release:** `[[c_abi]] struct` opts a passive, non-owning fixed-width/nested-record/one-level-pointer/direct-fixed-array field family into compiler-owned source-order layout and by-value or one-level-pointer `extern "C"` passage. Ordinary records, flexible or symbolic arrays, enums, bool/char, owners, references, and cleanup-owning values remain excluded. | choice | **bounded-first** | `S-ABI-01/02`, ADR 013, and `S-FFI-02` F1 are complete. A native C oracle proves size, alignment, offsets, nested records, direct C arrays, by-value return/passage, pointer mutation, and checked array reads/writes at O0/O3 and C++20/C++23. C array declarators are emitted directly rather than inherited from `std::array`; C layout remains unambiguous. `bool`, `char`, enums, owners, references, symbolic/zero arrays, and cleanup-owning values stay excluded. |
| `R-C-STRINGS` | **Partially closed:** `c_string` carries `const char*` at the C boundary, admits complete static string views only as contextual call inputs, supports nullable returns and `c_string*` out parameters, and exposes no unchecked read operation. Dynamic `std::string` borrowing and bounded read-back remain absent. | choice | **systems-ready** | `S-FFI-02` F2's boundary core is proved by the C17/C++20/C++23 oracle. The remaining owner slice must maintain a terminator and attach an invalidation-aware loan before `std::string::c_string()` exists; it may not be implemented as an untracked backend cast. `char` remains excluded from `[[c_abi]]` records. |
| `R-C-CALLBACKS` | **Partially closed:** named exact nullable native callback aliases, callback fields in passive native records, foreign callback transport, and same-thread non-capturing GTI free-function adapters are implemented. Callback values are not directly callable; captures, userdata pairing, arbitrary function pointers, and native-thread GTI entry remain absent. | lowering | **systems-ready** | `S-CALL-01` carries semantic target selection through HIR/MIR and emits a process-lifetime `noexcept` adapter whose defined-failure/native-exception firewall cannot cross C. Foreign-thread entry additionally requires the adopted concurrency capability/runtime rows. |
| `R-C-ABI-FAMILIES` | **Partially closed:** `[[c_opaque]] struct Name;` supplies nominal pointer-only handle identity, fixed arrays and callback fields are admitted in passive native records, `c_string` carries the NUL-terminated boundary, `[[c_array(count)]]` supplies bounded returned pointer-plus-count, and named callbacks supply the exact same-thread function-pointer family. General pointer nesting, annotated ownership transfer, array parameters, casts, varargs, native variables, alternate calling conventions, and C++ linkage remain absent. | choice | **bounded-first** | `S-ABI-03` generates one checked C17/C++20/C++23 adapter header; the 124-function GLFW 3.4 fixture and oracle cover all five public records, opaque handles, strings, returned arrays, callbacks, and Vulkan-facing declarations without object ABI, inferred ownership, or exception passage. Varargs, C ABI unions, bit-fields, packing, and general C++ linkage remain later breadth. |
...
| `R-CONTAINER-SURFACE` | **Partially closed:** vector has move-aware indexed insertion/erasure, but string mutation, iterator-position forms, and complete invalidation remain absent; vector is move-only, has no allocator policy, and does not promise C++ API parity. | library | **systems-ready** | `L-CONT-01` completes the reviewed surface needed by the renderer/game-loop workload. Implicit allocating vector copies are not required; duplication may remain explicit. |
...
| `R-TEXT-FORMATTING` | Canonical integer conversion/output and bounded sequential integral `{}` replacement are implemented; source/text encoding policy, dynamic owning/view conversion, numeric parsing, indexed/named/specifier and floating formatting, buffered/structured I/O, and stderr remain incomplete. | library | **systems-ready** | `L-TEXT-01` owns source/text policy and remaining formatting/parsing; `L-HOST-01` owns stderr and host streams. The landed slice uses an ordinary GTI formatter plus a bounded call-pack fold and scalar byte host entry; no public native buffer lifetime may leak. |
...
| `R-BUILD-CONFIG` | **Closed:** compile-time conditions compose target comparisons with source-ordered, valueless configuration flags using `defined`, `!`, `&&`, `||`, `#ifdef`, and `#ifndef`. Flags come from source, repeatable CLI `-D NAME`, or `[build].defines`; they never substitute tokens, and every branch remains parseable GTI. | choice | **implemented** | `S-CFG-01`; normative behaviour is in `docs/language/programs-and-targets.md`, with rationale and verification inventory in `docs/plans/conditional-compilation.md`. |
...
| §4.4 broad C interoperation | `R-NATIVE-RECORDS`, `R-C-STRINGS`, `R-C-CALLBACKS`, `R-C-ABI-FAMILIES` |
...
| Programs §6.2 target model and §6.5 native ABI | `R-TARGET`, `R-NATIVE-RECORDS`, `R-C-STRINGS`, `R-C-CALLBACKS`, `R-C-ABI-FAMILIES` |
