...
| --- | --- | --- | --- | --- |
| `R-EXEC-ORDER` | Only short-circuit and selected control-flow order is complete. Operand, argument, initialization, temporary, and cleanup order is not complete, so a transient borrow and overlapping mutation are conservatively rejected in one call regardless of written order. | lowering | **close-v1** | `D-EXEC-01` chooses the rule; `M-LIFE-01`, `M-EXEC-01`, and `M-BACK-01` must give it explicit MIR lifetime and emitted-order evidence before the overlap restriction is narrowed. |
| `R-TEMP-DROP` | Every temporary lifetime, partial-construction cleanup, compound-expression cleanup order, and cleanup at a checked failure is not yet authoritative. | lowering | **close-v1** | `M-OWN-02`, `M-LIFE-01`, and `M-FAIL-01`; verifier mutations and O0/O3 destruction traces must prove exactly-once obligations on every supported edge. |
| `R-FAIL` | Checked failures currently terminate, but report format, category/location token, exit status, cleanup, embedding, allocation failure, and worker failure lack one contract. | choice | **close-v1** | `D-FAIL-01` then `M-FAIL-01` and `Q-FAIL-01`; every existing trap family must map to one source-facing runtime entry. |
...
| `R-BACKEND-AUTHORITY` | The C++ backend still emits complete bodies from AST/semantic/HIR data, and some accepted evaluation/lifetime behavior is not yet executable MIR authority. | lowering | **close-v1** | `M-EXEC-01` and `M-BACK-01` start the migration. The 1.0 release gate requires every accepted observable order, failure, lifetime, and cleanup rule to be represented independently of C++, even while unconverted body families remain on an explicit compatibility path. |
...
| `R-EXCEPTIONS` | GTI has no language exceptions, catch, or native unwinding; recoverable APIs use `expected` and cleanup bodies do not throw. | safety/simplicity | **keep-v1** | `D-FAIL-01` defines termination/embedding without exceptions. Exception syntax or cross-ABI unwinding needs a new post-1.0 design row. |
...
| `R-GLOBAL-OWNERSHIP` | Reference, borrowed-state, unique-owner, and cleanup-owning globals remain unavailable; mutable ordinary globals exist only in the current single-thread profile. | choice | **bounded-v1** | `C-GLOBAL-01` defines the adopted concurrent-profile rule before 1.0. Broader process-lifetime ownership requires a new row after global initialization, shutdown, failure, and foreign-thread participation are explicit. |
...
| `R-CALLABLES` | Lambdas use explicit typed parameters/results and immutable copy snapshots. Capture defaults, reference/init/move capture, mutable closures, arbitrary results, callable references, field/global storage, and general escape are rejected. | proof | **bounded-v1** | `D-CALL-01` defines one model and `L-CALL-01` ships the owned callable/move-capture minimum needed by v1 algorithms. General type erasure, reference capture, and unconstrained escape require a new post-1.0 row. |
| `R-FUNCTION-VALUES` | Function names must normally be called; ordinary function items and exact function-pointer values are not first-class. | lowering | **post-v1** | `S-CALL-01` owns the first exact non-capturing function item only for a demonstrated C callback boundary. General function values need a separate callable client and row. |
...
| `R-C-CALLBACKS` | GTI cannot expose function pointers/callback thunks, retained userdata, or native-thread entry. | lowering | **post-v1** | `D-CALL-01` then `S-CALL-01` for same-thread callbacks. Foreign-thread entry additionally requires the adopted concurrency capability/runtime rows and registration lifetime proof. |
...
| `R-RECOVERABLE-ALLOCATION` | `make_unique` is type-level infallible and there is no complete `try_make_unique`/shared allocation family. | library | **close-v1** | `D-FAIL-01` and `L-OWN-01`; v1 owner factories must expose the selected recoverable allocation path without raw ownership construction. General allocator customization remains post-v1. |
