...
| `R-TEMP-DROP` | Typed lexical and materializing-value obligations, reverse normal cleanup, ownership transfer, and path-conditional availability are authoritative in HIR/MIR. Ordered compound materialization, partial-construction rollback, and checked-failure cleanup remain incomplete. | lowering | **systems-ready** | `M-LIFE-01` and `M-OWN-02` are done. `M-EXEC-01` and `M-FAIL-01` provide the remaining order/rollback/failure facts and matching M-BACK slices provide production authority. |
