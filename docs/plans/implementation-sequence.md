...
| --- | --- | --- | --- | --- | --- |
| 1 | `M-BACK-02` final cutover | **active** — 2374 of 2482 corpus bodies emit from verified MIR (oracle census, 0.288.0: fixed-array element aggregates (+2) — an Aggregate compute over a fixed-array type spells its staged elements in operand order inside the braces, exactly like the compatibility aggregate initialization, and the cli fixed-array bounds fixture migrates to the structured GTI-R0007 report as the aggregate-initialized entry joins verified emission; 0.287.0: the substituted default construction (+2, completing 31-exact-constraints) — a type parameter's T() monomorphizes to the targetless default-construction intrinsic, which spells the concrete value initialization inline at its consuming return on both forms (or into its declared local otherwise), and a call-input stage sourced from a declared class value copies into its own declared local under the copyable row — the gate/text mirror gap there was caught by the per-source compile rung; an attempted moved-load and declaration-class-return pair was REVERTED when the temporary-lifecycle fixture pinned extra ctor/dtor trace lines, naming the fifth pin family: any widening that adds declared class locals or changes temporary materialization runs cli_workflow early; 0.286.0: copy loads under the copyability row (+2 in 31-exact-constraints) — the type row gains the copy proof (usable copy constructor and copy assignment from the semantic lifecycle), a class Load with a Copy operand declares and assigns under it in both mirrors, and the generic Load spelling takes the storage place value so lifetime-slot sources bind through their accessor (the third slot-accessor defect, caught again by the per-source compile rung); 0.285.0: floating signature positions (+4: the whole 11-overloads chain and another binary64 body) — floating widths are passive pass-by-value scalars at every signature position under the same nontrapping ruling, so float-parameter and float-returning declarations join the declaration-keyed, transformed-sibling, and specialization selectors; 0.284.0: floating arithmetic joins the vocabulary (+1 in 47-binary64) — IEEE-754 nontrapping results are not defined runtime failures (docs/language/execution.md), so a floating Add/Subtract/Multiply/Divide/Negate spells the terminal compatibility helper on both forms whether or not MIR carries a failure site, any paired failure edge is a dead plain goto at the invoke gates and text, and floating computes are excluded from the failure-status declaration pass; 0.283.0: boundary and class-value vocabulary consolidation (enabling; census holds while caller chains peel through the fixpoint) — the transformed boundary accepts string-view results at the failure-form return gate, the callee-return gate, and the failure selector's return clause; the class-value enumeration gains three declared stories with gate/text mirrors (never-raising callee results, constructor invocations, and the targetless empty call that an explicit no-constructor default construction lowers to, whose call text spells the value-initialized temporary); each measured carrier chain (11-overloads, 26-stored-references, 31-exact-constraints) now stops only on float checked arithmetic, copy-load class values, or remaining callee admissions; 0.282.0: the VirtualDispatch capability opens (+2 in 25-polymorphic-ranges) — the capability row names cpp_virtual_member_v1 (the emitted in-class definition carries the language's own dispatch), a virtual call site spells the ordinary member call through the base-typed receiver and demands only the callee's body-name row, virtual and override method bodies join the declaration-keyed and transformed-sibling selectors as ordinary members, and pure virtual contracts are excluded at the admission authority itself — the capability row alone had flipped their stub bodies into both admission sets, and the specialization surfaces defined specializations of pure declarations (caught by the per-source compile rung, another blind spot the corpus oracle cannot see); 0.281.0: fixed-representation initializer coverage (+12) — a union or C-ABI record keeps its representation token-fixed, so its verified field-initializer schedule is covered by the definition exactly when every field is the bare default (the ordinary route's shipped no-text spelling), and the capability-class arm accepts all-bare schedules alongside empty ones; any staged initializer text on a fixed representation still declines fail-closed, protecting bridge-header token equivalence; 0.280.0: the initializer schedule gains null and empty-aggregate fields (+7 field-initializer bodies) — the null literal spells nullptr for pointer-typed uses in the shared literal writer, an empty Aggregate compute spells the row type's value initialization, and the schedule's fifteen decline points are permanently instrumented behind GTI_PROBE_TRACE (INIT traces) alongside the general failure selector's SELF traces; 0.279.0: checked compound assignments join the failure form (+7, all in 58-formatted-output: the format chain through try_println) — the status-returning mir_checked_compound_*_v1 family composes exactly the terminal add_assign contract (base operation checked in the C++ common type, then the checked narrowing commit), so each failure mode carries its own record code and the cli compound-narrowing fixture migrates to the structured conversion report; the detector accepts bare scalar locals and the dereference-projected reference place whose carrier the store path already spells, and the borrow-staged call argument takes the storage place value so lifetime-slot arguments bind through their accessor; 0.278.0: the Payload capability opens (enabling; census holds) — payload enum rows copy the compatibility variant record inventory (__gti_variant_<index> alternatives with per-field types), the Payload capability row names cpp_variant_record_v1, PayloadConstruct/PayloadExtract verify and spell through the copied variant row (construction wraps the variant record, extraction reads the proven alternative field), a payload-enum switch discriminates on the variant record's alternative index with copied-index cases, and an enum result crosses the transformed failure boundary when the copied row proves every variant field scalar or view — the remaining sum-type blockers are the RawMemory capability (unsafe union reads) and entry-chain containment, so the vocabulary waits dormant exactly like the 0.213.0 Expected convention did; the same slice widens the transformed boundary to Expected-of-class results carrying the 0.215 boundary proof at both the ff return gate and declaration eligibility, and instruments the general failure selector (SELF traces) — the measured 58-formatted-output chain is now fully attributed: format's ff admission survives to the ADR 017 fixpoint and drops only because validate_format_pattern/append_format_argument hold checked stores into storage interiors, so the P-STORAGE backend rows are the binding campaign for that pool; 0.277.0: direct borrow arguments — a BorrowRead/BorrowWrite call operand with a valid place is its own schedule entry, since forming the place lvalue has no effects; the argument text spells the storage place value so lifetime-slot places take their accessor, widening the exact-input match, the compiler-generated schedule branch, and the staged-operand walk (+4: callable-parameter borrows in 28-callable-parameters, the accumulate chain caller in 44-bounded-requires, and a stateful-callable borrow in 52-stateful-callables); the paired-plain terminal-helper widening was attempted and REVERTED — a paired site carries the record protocol whose report names the exact source site, which the terminal helper cannot spell, and the cli arithmetic fixtures pin file:line plus exit 70, so only unpaired sites are the terminal-helper shape; 0.276.0: sequenced moved arguments — a Move-defined value whose single record is one call operand declares at its move as `auto v = std::move(place);` and the argument consumes std::move(v), keeping MIR's move-before-call order where the rejected inline spelling raced sibling arguments under C++'s unspecified evaluation order, with auto requiring no representation row for move-only iterator types; the nested contained-member exemption joins the plain class-result gate (+1 in 44-bounded-requires, with sum's whole move schedule now verified and the chain waiting only on accumulate's iterator-loop call-input schedule); 0.275.0: per-site wrapper containment — a plain call whose failure continuation is provably unused (transparent or flow-discharged propagation, any paired invoke's else path only cleanup glue ending in the terminal failure family) takes the ff-admitted callee's terminating plain name, the valueless success return of an Expected<void, E> body spells the engaged empty value, and the four backend fixture ledgers migrate their newly emitting callers with the fixture entry mains joining the selections; the true convention pins hold — a handled site never matches (+2: the mains of 07-generics and 37-tcp-socket-owner); 0.274.0: the call-result loan capture and the containment convention — a plain call pairing a CallResult loan now spells the loan capture over the call expression instead of a value assignment that left the pointer null (the defect behind two oracle rejections), a paired result value may have no direct consumer, and the closed narrowing compound assignment carries its complete vocabulary through the terminal helper family; the per-callee wrapper-containment widening was attempted a third time on the fixed base and the suite's transformed-convention pin rejected it — a handled caller needs the sibling's error value, so any future widening is per call site; count unchanged, the capture is the prerequisite the caller cascade builds on; 0.273.0: the multi-site constructor family emits on the healed base — a checked compute or storage append whose site carries no invoke edge spells the terminal compatibility helper in either form (an unpaired failure-form status write would silently swallow the failure the helper must report), literal constant operands spell inline, and the string_view constructor family's allocate/publish/append schedule now lands the allocation in its field before the loop — the exact divergence the oracle rejected pre-healing (+3: the view-constructor pairs of 20-strings, 40-loan-flow-edges, and 43-program-arguments); 0.272.0: constructor-initializer publication heals — an unconsumed initializer argument now publishes into its bound field through the initializer's HIR argument, closing a latent drop in the allocate-empty family that was behaviorally masked only because a zero-capacity allocation equals the default, with reference-storing initializers excluded (their stored loans bind in the member initializer list) and a fail-closed gate demand on the field binding; count unchanged — this is a soundness release; 0.271.0: the plain storage-allocation form opens — a checked conversion compute spells the terminal numeric_cast in the success form, a prefix-storage allocation with its one exhaustion site publishes into its storage-typed result through the terminal allocate_prefix_storage helper exactly like the compatibility initializer-list call, and both containments satisfy the checked-failure edge demand (+9: the allocate-empty constructor family across 20-strings, 38-vector-emplace, 40-loan-flow-edges, 43-program-arguments, 44-bounded-requires, 52-stateful-callables, and 58-formatted-output); 0.270.0: bounds-checked subscript accessors emit — a Borrow of a This-rooted field element place carrying its one bounds-failure site and publishing the return loan fuses as the terminal array_at accessor at the consuming return, with the element place alias-free and the loan pointer-free, exactly the compatibility subscript body in both forms (+4: the operator[] pairs of 14-operators and 17-standard-array); 0.269.0: the program-initialization contract opens — the verified module schedule authors every program-storage definition's initializer text, C++ static initialization in plan order performs the module body's work under the new cpp_static_initialization_v1 capability row, the hosted schedule accepts the optional leading CallProgramInitialization operation, and class-static and wrapped internal-linkage storage rows land (+4: the module body and hosted adapter of 24-static and 55-owned-callable-transport); 0.268.0: enumerator storage rows and the failure-free hosted schedule — a Symbol-rooted place reading a non-payload enum constant now carries a storage row spelling the enum type's qualified constant, and a program entry that cannot raise proves the two-operation CallEntry/ReturnEntry hosted schedule so its bare call/return adapter claims verified-MIR authority; the admission probe traces carry each issue's detail text (+5: the mains of 21-scoped-enums and 22-switch, and the hosted adapters of 06-namespaces, 36-c-abi-sockets, and 50-opaque-native-handles); 0.267.0: stored-loan construct returns emit — a body returning a class that stores a borrowed reference fuses its Construct at the plain return exactly as the failure form already did, the Return loan is erased into the constructed value per ADR 018 with no pointer local, and the nested contained-member call machinery landed behind the containment proof for the coming caller cascade (+7 across 26-stored-references, 38-vector-emplace, 44-bounded-requires, and 52-stateful-callables — the vector begin/end iterator family); 0.266.0: owner-borrow accessors emit — a unique-owner `operator*`/`operator->` body whose single owner-borrow intrinsic call reads a This-rooted owner field through a staged Load publishes as the fused `owner_access` spelling at its consuming return, with the staged load, both loans, and the borrow silent, resolved through the loan parent chain (+7 across 13-ownership, 19-auto, 35-conditional-expressions, 38-vector-emplace, and 42-exclusive-reborrows); 0.265.0: the pack-forwarding pilot completes — a parameter-pack declaration is specialization-eligible because each instance arrives with flattened concrete parameter types, the specialization signature names its substituted arguments explicitly where deduction cannot recover them, and the pack place and value spell nothing while the allocation call carries the flattened parameters (+5: the make_unique instances across 13-ownership, 16-move-generics, 19-auto, 35-conditional-expressions, and 42-exclusive-reborrows — the first pack bodies ever emitted from verified MIR); 0.264.0: the class-return specialization surface opens under the enumeration's guard — a specialization may return a class whose row carries the boundary proof, the pack allocation spells the backend helper over the flattened parameters with the expansion and pack place silent, the boundary wrapper forwards consumable owners by move, and a class-valued return published through a loan stays fail-closed (copying the borrowed referent is rejected and moving from it is unsound); the plain-form return-move fusion was tried and withdrawn when the oracle showed the drop ordering reads a destroyed slot, so the failure form keeps sole ownership of move publication (+5: a transfer body in 16-move-generics and four formatting bodies in 58-formatted-output); 0.263.0: ordered moved-place chains — a move flowing through plain value staging into one construction in its own block, with nothing between touching the source place, spells std::move over the place at the construction argument and every link stays silent; the first cut without the order proof produced behavioral divergence in two sources and was rejected by the oracle, so the shipped arm carries the same-block source-untouched scan and a traced match discipline (+1: the main in 16-move-generics; one chain match corpus-wide, exactly the intended one); 0.262.0: the pack-forwarding vocabulary opens — the bounded forward-once shape is analysis-ready and admitted (an operand-less PackExpansion whose TypePack value feeds exactly one unique-owner allocation, with the pack parameter place and value spelling nothing), the class-value flow enumeration replaces the blanket value skip with declared-or-no-local proof per defining instruction (parity held at 2277 first cut, then the contained-plain-result declared story landed on both gate and text), and generic instances whose type parameters C++ cannot deduce from parameter types carry explicit substituted arguments in their call spellings through the new MirFunctionInstance typeArguments (+1: the main in 13-ownership calling make_unique; the Move-chain-into-call-argument family named next at the enumeration, measured on 16-move-generics); 0.260.0: admission diagnostics land permanent (analysis not-ready and specialization-skip traces, the enriched terminal-place probe) with the plain class-call-result guard and a dormant return-fusion vocabulary — the class-return specialization surface stays closed after measurement showed the value rows admit every class-typed value silently while only some defining instructions spell without a local (three examples went unbuildable under the trial widening), so the class-value flow audit is the recorded precondition for map_value and the class-return family; 0.259.0: move-staged temporaries — a value-staged call-input temporary now also accepts a source moved through its staged value (spelled std::move(source) at the consuming call, with the feeding Move silent), and a moved temporary receiver names its source place under the Consuming mutability so a consuming call operator spells std::move(place).member(args) exactly like the compatibility __gti_invoke friend (+1: run_once in 53-consuming-callables; run_once_with declines at the member mutability gate pending the mutable-on-moved convention, and map_value's second instance stalls before the probe layer — next diagnosis); 2276 at 0.258.0: by-value copy staging and canonical slots — a CallInput copying a source place into a bare class-typed temporary that nothing else references never materializes (the consuming call spells the source place and C++ copies at the call boundary, exactly like the compatibility call), and duplicate bare binding places share their binding's canonical lifetime slot so the construct, every read, and the destroy touch one slot — the oracle caught the first cut reading the never-constructed duplicate (+2: the main and its lambda in 29-callable-predicates); 2274 at 0.257.0: loan-staged call inputs — a CallInput carrying a Loan operand into a receiver-carrying member call spells the dereferenced pointer carrier (ADR 018 §4) at the consuming call, the staged reference value never declares, and the member-call convention otherwise unchanged owns the call — this makes a deduction-called functor instance with a reference parameter per-instance admissible (+1: apply_twice binding the Replace object in 28-callable-parameters); the overlay-precedence experiment for the shared failure template was withdrawn after it split declaration/definition authority — the declaration pass promised per-instance specializations while the definition flow took the callable-template route, measured as an ex29 undefined-symbol failure — so per-instance specialization remains the sole owner of nameable concretes and the shared-template route stays fenced to overlay-row-creating bindings; 2273 at 0.256.0: the may-raise deduced-callable template surface lands dormant — the transformed sibling template assembles per-declaration under overlay rows through the failure-form probe and text (the callable-template flag now serves both forms), the boundary wrapper template keeps the original name and terminates on a propagated record, and the route declines any declaration that creates no overlay row (a scalar generic sibling would displace its own compat linkage — measured as an ex58 undefined-symbol failure and fenced) — dormant because every corpus candidate either binds mixed nameable-and-unnameable callables (the recorded split-emission design) or no callables at all; 2273 at 0.255.0: the slot-state flow completes for argument-carrying constructions — a slot construct spells its staged arguments, and a class value living in a declared local (a transformed callee's published result) engages its lifetime slot by move at the consuming Initialize, with the reparent comment kept exactly where the paired construct built the value in-slot — the oracle's earlier rejection traced to the missing engagement, reproduced at runtime and fixed at the discriminator (+2: the string mains in 20-strings and 23-direct-initialization); 2271 at 0.254.0: the class-value vocabulary lands enabling — type rows carry the 0.215 boundary proof (usable default constructor and move assignment, recorded where the row builder holds the semantic lifecycle because MIR records no special-member status), a class value declares exactly when a value-producing construction or a transformed callee's class result defines it and the slot protocol does not own it (the slot-consumer exclusion the oracle demanded of the first cut), the construction spells assignment into the declared local, and the transformed-call caller admits class results through the same proof — the chains now stack at the slot-construct-with-arguments shape, whose first cut the oracle also rejected pending the reparent-pairing investigation; 2271 at 0.253.0: field-less stored loans join the loan vocabulary — a borrow-carrying object value holds its borrow internally, no pointer binds, and admission requires exactly that nothing roots a place at the loan (constructor field pairing unchanged) — enabling: the four blocked mains advanced from the loan gate to the class-value construction layer, whose first cut the oracle rejected for hijacking slot-reparented constructs (the re-cut needs the slot-consumer exclusion; the parked patch and finding are recorded); 2271 at 0.252.0: parameter loans join the ADR 018 vocabulary — an entry loan with one carrier over a reference-typed parameter place aliases the pointer carrier the reference-local prelude already binds, callable invocations accept loan operands and spell the dereferenced carrier, and the store path gains the dereference-projected destination the read path always had — landing the mutable-reference callable chain (+3: the Replace operator() family in 28-callable-parameters and two format members in 58-formatted-output; the oracle caught the first cut's mir-build-failed when the admission unmasked the missing deref-store spelling, and a speculative row-free store decline was measured regressing lambda bodies and removed); 2268 at 0.251.0: the fixed-array size read joins the builtin-member intrinsic convention (third application: ArraySize recorded at analyzeArrayMemberCall, pure effects, the shared receiver-intrinsic probe/text arms spell .size() on the staged array place), completing 45-layout-queries at 28 of 28 — the first fully-emitted example — and a user-reported GTI-B0001 ICE is fixed at its semantic root: a constructor initializer accepting a range-checked constant into a wider integer field now re-types the literal itself (the accepted set is exactly direct possibly-grouped literals, so no conversion machinery is needed and the stage verifier's operand-type demand holds; non-literal and negative arguments were already rejected), with the seven-line repro pinned in mir_backend_tests; 2267 at 0.250.0: the expected extractions join the builtin-member intrinsic convention — analyzeExpectedMemberCall records ExpectedValue/ExpectedError, the receiver-intrinsic probe arm admits their single wrong-state site because the spelled member contains it terminally, both invoke forms treat the extraction producer as a dead else edge, and the text spells the staged place's .value()/.error() exactly like the compatibility route — landing 08-expected's main (+1; the extraction vocabulary also unblocks later chains as their other gates clear); 2266 at 0.249.0: the value-level view element read joins both forms — a Compute Index over a staged view value and index spells the terminal string_view_at helper exactly like the place-projected form, and its paired invoke edge is a plain goto on both forms because the helper never returns on failure — landing the prelude string's equals and releasing the fixpoint-held == and != operators (+16: +4 each across 20-strings, 40-loan-flow-edges, 43-program-arguments, 58-formatted-output); append still stacks the discharged-add layer; 2250 at 0.248.0: the builtin string-view members carry their identity into MIR — analyzeStringViewMemberCall records a ResolvedCallInfo with the new StringViewSize/StringViewEmpty intrinsics (the calls name no declaration, so without the record the lowered Call was target-less and unnameable by any backend), the effect table classifies them pure, and the verified emitter spells the staged view place's member directly on both text forms while the compatibility route keeps its untouched generic member spelling — enabling: the prelude string's equals and append advanced from the receiver-target gate to the last compute layer (the value-level view element read and the discharged add), with the == and != operators' failure probes now passing behind them; 2250 at 0.247.0: the class-valued publication construct consumes borrow-staged arguments — the staging window treats it as a consuming step, its arm consumes each staged operand from this block or the unique invoke-predecessor, the block-end hand-off accepts staged-argument uses alongside receivers, and the publication text spells a staged place like a call argument — enabling-only: the probed member now waits on the class-result boundary (its returned class fails the default-constructor/move-assign requirement, the recorded placement-new publication design); the corpus-wide permanent-trace census then ranked the remaining frontier — 60 function bodies decline both forms, led by the receiver-call target gate (11: the prelude view-member family fn 23/47 recurring across five sources — target-less receiver calls on view locals, the deduced-callable provenance campaign's member variant), then parameter-shape and class-value construct gates, with the silent remainder split between fixpoint drops, surface-less admissions, and the pack/initialization campaigns; 2250 at 0.246.0: the plain shape closes over terminal containment — a propagating construction and a site-carrying checked conversion are vacuous-else invoke producers in the success form too (the construct terminates at its own site, the conversion spells the terminal numeric_cast the compatibility route uses), and a plain-shape propagate block admits with a genuine unreachability proof (a walk from the entry following every edge except invoke else edges, which the producer rules prove dead, must never land on it) — and the admission selector gains the member-sibling post-pass the widening exposed: a may-raise member spellable in both forms prefers its failure admission exactly where a failure surface will emit it (the declaration-keyed concrete selector or the ADR 019 specialization, consulted after the sets finish building because both read the admission state), since a plain admission there would drop the transformed sibling and cascade its containing callers out of the ADR 017 fixpoint — the bisect that found this walked the selector's either/or from a 12-body ex58 cascade to the exact member/free asymmetry (+8: checked-conversion plain bodies in 14-operators and singles across 38-vector-emplace, 41-owner-dependencies, 43-program-arguments, 57-contextual-array-arguments); 2242 at 0.245.0: the staged-borrow discipline generalizes in two oracle-proven steps — each call consumes exactly its own staged inputs with borrows for a later same-block call staying pending, and a staged borrow hands off across the invoke edge when every leftover's consuming call sits in the success target, the target has the invoke as its only entry, and the else path never references the staged places — enabling-only while the format-family chains accumulate; 2242 at 0.244.0: the move-publication return completes soundly — the publication moved to the Move site itself, where MIR consumes the source and before any later drop of the moved-from local (the publish-at-return ordering read a destroyed slot, the exact abort the oracle caught), the universal reachable-only-drops scan vets every path leaving the Move's block, terminator uses generalize to any number of Returns of the value, and the paired Return emits no assignment — recovering the string member across 20-strings, 40-loan-flow-edges, 43-program-arguments, and 58-formatted-output with the oracle green over the reordering; 2238 at 0.243.0: the move-publication return lands in its oracle-proven same-block form — a class-valued failure Return whose value a Move produced in the Return's own block, nothing between them touching the source, the value's uses exactly the Return plus at most one drop-only Value-rooted place — publishing `std::move(<source>)` through the out-parameter with the Move and residual drop as comments; the callable-staging window admits Lifecycle and EndBorrow; and the text-side Invoke emission gains the vacuous-else arms the probe already admitted. The differential oracle EARNED ITS KEEP twice this increment: it caught an unrestricted move-publication reading moved-out slots and a success-form Move-comment leak producing garbage exits (a +4 census that was behaviorally wrong), and the cross-block invoke-adjacent widening was reverted as unsound pending a real slot-state flow proof — the bisected re-add with a full oracle per piece is now the mandatory protocol for stacked probe/text edits; 2238 at 0.242.0: the invoke-producer rules accept two vacuous-else shapes — a discharged storage read (no site: flow analysis proved the bound) and a propagating construction (constructor failure terminates at its own site until the constructor failure ABI exists) — mirroring the analysis's exemptions, and the permanent probe trace gained an RAII identity scope so nested containment probes no longer misattribute decline prints (all earlier owner-attributed census data was suspect; the rebuilt census confirms the top blocked chain is the string member whose staging window still refuses its Drop/EndBorrow/Lifecycle interior, the recorded next mini-campaign); 2238 at 0.241.0: comparison operands accept literal constants (spelled inline exactly like the checked detectors' constants); the member-callee containment analogue joins the plain-call rule and the invoke pre-exemptions (success-shape proof only, cycles fail-closed, receiver spelling verified present on the plain-call text path) — the first-family suite caught and reverted an either-form trial that would have displaced the transformed propagation convention, so failure-admitted callees keep their transformed calls and only the subscript-member resolver accepts the wrapper form, where no transformed alternative exists; the permanent-ordinal layer census then established the endgame shape: twenty-eight genuinely both-form-declined function keys, led by target-less callable-parameter invocations (the deduced-callable provenance campaign, free and member variants), so the single-rule frontier is exhausted and the remaining pools are the designed campaigns; 2238 at 0.240.0: the class-subscription vocabulary lands enabling-only — an Index-projected place over a sibling class-typed local resolves its subscript member through the existing body-name rows (containedSubscriptMember: exact receiver mutability and index parameter type, unique match, and the member's own emitted body proving terminal containment, cycles fail-closed), reads spell the compatibility read_only_receiver member call and stores the mutable member call on both text forms; the subscript-blocked entry advanced from its place rule to the callee-chain gate, so the count holds while the entry chains accumulate; the four new decline points joined the permanent trace as stable ordinals 121-124; 2238 at 0.239.0: the string-view element vocabulary lands enabling-only — an Index-projected place over a sibling string-view local is accepted by the probe and its Load spells the terminal bounds-checked string_view_at helper the compatibility subscript uses, on both text forms; the view-reading bodies stack further gates, so the count holds while the pieces accumulate — and the probe's decline points now carry permanent GTI_PROBE_TRACE-gated tracing with stable ordinals, so decline censuses no longer need rebuilt instrumentation; 2238 at 0.238.0: the callable-staging window admits Move instructions — a Move between a staged callable and its consuming call is inert argument staging that neither raises nor observes, so the deferred receiver spelling stays exact — landing a stateful-callable map body in 51-callable-value-results (the full-occurrence probe trace also corrected the census method: bare-line-only instrumentation missed braced declines, so zero trace lines never proves acceptance); 2237 at 0.237.0: slot-allocated parameters and class-typed callable specializations join the vocabulary — a function body's slot-allocated parameter constructs its slot from the argument in the prelude (constructor and lambda bodies keep the compatibility route while their moved-in class values still flow through undeclarable value locals), and the per-instance template-specialization route accepts concrete class-typed parameters (a deduction-called declaration's stateful callable is nameable, and the admitted body's analysis already demanded its rows) — the probe now passes the stateful-callable bodies in both forms with the remaining yield gated in the route layer (deduced-callable increment 2, task #41); 2236 at 0.236.0: the storage growth vocabulary — an AllocatePrefixStorage call may carry its modeling receiver (a raw borrow of a spellable place that never spells, exactly like the other storage intrinsics), and the store-consumed storage value drop: a growth step moves the replacement out of its staging slot and stores it into the receiver field, the store is the value's single body-wide consuming use and spells as a C++ move-assignment, so the Drop of the moved-from value afterwards is a no-op by representation and spells as a comment (proof: projection-free Value-rooted storage place, unconditional non-failure lifecycle, exactly one read — an Initialize/Assign preceding the Drop in its own block — and no other place rooting at the value) — releasing the string/vector growth chains (+29 across 20-strings, 38-vector-emplace, 40-loan-flow-edges, 43-program-arguments, 44-bounded-requires, 52-stateful-callables, 58-formatted-output); 2207 at 0.235.0: the failure-sibling naming authority — cppMirFailureSiblingSpelling, shared by the compatibility signature/wrapper/adapter emission and the verified caller spelling — carries plain and mangled member names with the __gti_mir_failure suffix directly and derives a mangled token sibling (__gti_mir_op_eq__gti_mir_failure and kin) for structural operator bridges, whose real C++ operator names cannot carry a suffix; an operator outside the token map keeps the compatibility route fail-closed, and the 0.222.0 bridge decline now admits exactly the mappable bridges — recovering eight bridge-operator bodies (two each in 20-strings, 40-loan-flow-edges, 43-program-arguments, 58-formatted-output; the fourteen remaining bridges stack further gates); 2199 at 0.234.0: the owned-arguments hosted-startup schedule joins the verified vocabulary — cppMirHostedStartupOwnedArgumentsSchedule verifies the 36-operation marshaling plan exactly (detect-validated count and conversion, propagating vector construction, the per-argument view/string/append loop with its drop/end failure-cleanup envelope, and the entry call, each failure routed to a distinct record and terminally contained under immediate-exit-70), the emission analysis accepts hosted bodies under either schedule with the owned marshaling obligations covered by the plan's own cleanup envelope, and the argc/argv main carries the hosted-entry-v1 marker as that body's complete emission — landing 43-program-arguments' hosted startup; 2198 at 0.233.0: the moved-out owner-drop convention — a Drop of a unique-owner binding whose value an earlier same-block Move consumed, with every lifecycle event unconditional, spells as a comment because scope-end destruction of the moved-from local is a no-op by representation; unique-owner parameter locals move-initialize from their arguments and a unique-owner store spells std::move (the store is the value's consuming use); constructor instance-eligibility replaced its blanket drop-obligation/cleanup-boundary refusal with a path-shape proof — every boundary Normal-kind, every lifecycle event unconditional non-failure — leaving each Drop's representation to the body-text authority; and the probe declines slot-allocated parameter places, which have no argument-construction staging in the prelude (the census that exposed the gap: two class-owner slot-parameter constructors in ex16/ex37 produced ill-formed text before the decline landed) — recovering five moved-in owner constructors across 13-ownership, 16-move-generics, 19-auto, 35-conditional-expressions, and 42-exclusive-reborrows; 2193 at 0.232.0: the constructor selector's inline owner-gate duplicate dissolved into the shared generalConstructorOwnerEligible authority, which also stops refusing active-cleanup owners — cleanup governs destruction, not construction, and the instance-eligibility half already refuses failure records, drops, and cleanup boundaries inside the body — landing the owned-class constructors; 2190 at 0.231.0: program-initialization steps for plain namespace globals carry their own storage rows — a constexpr global's reads are frontend-substituted so nothing else demanded them — the schedule walk follows the plan's Goto-chained units and constant operands, and a module body whose staged fields are all data-only constant steps is completely emitted by the declarations above it and owns its marker; 2187 at 0.230.0: the verified initializer schedule walks the terminally-contained invoke chain, so a computed in-class default like file_handle's negate(1) spells its compatibility helper expression and the field-initializer body emits across every example (+57); three improvement-fired exact-count pins migrated with the recovered bodies named; 2130 at 0.229.0: the terminally-contained plain-callee convention generalizes the deduced-callable exemption to concrete free functions — a may-raise free callee whose own body proves the plain success shape is called by its plain name from either form, its invoke edge is a plain goto, and the admission fixpoint accepts success-form-admitted free callees — releasing the entry bodies the fixpoint had dragged corpus-wide (+65, the largest slice since hosted startup); the first-family pin migrated five-to-six naming the recovered char println; 2065 at 0.227.0: floating literals joined both text forms through the shared byte-exact bit_cast authority that the compatibility emitter now also consumes, and the mir_checked_* detector family fails closed on non-integral operands after the widening exposed a float-to-int8 conversion admitting against integral-only helpers; 2064 at 0.226.0; 1921 at 0.199.0; 1276 at 0.197.0, 1503 at 0.198.0). 0.199.0 emitted the native-boundary pool: a non-source definition's verified boundary shell (one reachable empty block ending in Return or Unreachable) marks `native-boundary-v1` at its declaration surface — the spelled extern signature or the runtime-header-backed intrinsic site. Oracle: eight configurations (O0/O1/O2/O3 × C++20/C++23) all agree — 57 sources, 0 disagreement, 0 not comparable, 1921 of 2482 bodies under comparison, 0 uncovered sources — with per-standard emission (the helper now emits C++20 text for the C++20 legs, closing a blind spot where every leg compared C++23 text), and the full suite is green under both macOS/AppleClang and ubuntu-24.04 g++ 13.3 with libstdc++/LLVM 18. Vocabulary since: discharged storage reads accepted by analysis with the loan pairing staged for the reference-return ABI; Expected capability row plus both expected operations (analysis demands after the transparent direct-call propagation slice: 152 call-input schedules, 151 capability — CallableDispatch 55/Closure/HostedEntry/RawMemory 10/VirtualDispatch 9 — 82 checked-failure control flow, 52 constructor-rollback, 36 constructor checked-CFG, 57 hosted-startup bodies, 8 lambdas, 5 non-empty modules, per the instrumented analyzeProgram census; remaining bodies are blocked by conjunctions of these pools, so emission holds at 1921 of 2482 until whole chains clear; after the staged-receiver schedule rule the call-input pool measured 152 to 82, and the HostedEntry capability row landed with entry emission itself gated on a measured optimizer-parity dependency — verified MIR lacks the comparison/logical constant folds and branch-on-literal elimination the compatibility O1 contract pins, trialed and reverted at fa00e7c). 0.200.0 delivers fold-parity part 1: the ComputeFold provenance (mir-v32) folds boolean-producing comparisons and logical-not through the single evaluateMirComputeFold authority under the existing shadow-agreement discipline, with in-body and coherence replays, fail-closed forged-literal rejection, and O0 byte-identity pinned; 0.201.0 delivers part 2's core: branch-on-literal elimination (mir-v33) — a Branch whose condition folds to a literal bool rewrites to a Goto carrying BranchFold provenance, replayed by the in-body verifier from the retained condition and by the optimization-coherence check against the exact source branch, with reachability recomputed truthfully and the fold scoped fail-closed to bodies without loans, drop obligations, cleanup boundaries, failure records, or frozen program-initialization steps (the lifecycle and module interactions surfaced by cli_workflow and mir_program_initialization are the recorded widening work); 0.202.0 iterates the fold pass to a bounded fixpoint under one transform report, so comparison, identity, and branch folds cascade — the full `(1 < 2) && !false || false` chain now constant-folds through verified MIR at O1; 0.203.0 re-opens no-argument entry emission: the cli optimization pins migrated to MIR-authority evidence (O0 keeps the staged compare, O1/O3 fold it away), and entry emission surfaced and fixed a real call-target row defect — a C-linkage declaration inside a namespace now spells its namespace-qualified call target, since extern "C" affects linkage rather than C++ name lookup. Corpus emission 1925 of 2482 (the four provably failure-free mains; may-raise mains wait on the failure-form entry). 0.204.0: the failure-form entry was trialed and reverted on measured evidence — emitted mains left engaged lifetime slots on the propagate path (prefix evidence guards aborted 'slot escaped without Drop'), so failure entries wait on the cleanup envelope; the trial also surfaced and fixed a real checked-negation helper defect (a wider signed operand now negates in its own domain before the conversion range-check), and the general failure entry adapter is in tree dormant. docs/architecture/mir.md now records the rewrite-provenance contract (IdentityFold/ComputeFold/BranchFold). 0.205.0 completes the failure-form entry: failure-cleanup drops destroy their lifetime slots (the comment-only stub was the slot-escape root cause), the failure selector admits no-argument entries through the general failure entry adapter, and the affected pins migrated to the surfaces that now fire first — the prefix evidence battery pins the defined storage contract (GTI-R0007/GTI-R0010 records at exit 70) with the sealed guards as defense in depth, and the pipeline/layout text pins assert MIR-authority fold and staged-literal evidence. Corpus emission 1931 of 2482; oracle 57/0/0 re-verified at O0-c++23 and O3-c++20. 0.206.0 emits the no-argument hosted-startup bodies: cppMirHostedStartupNoArgumentsSchedule is the single authority verifying the four-operation plan (CallEntry with propagated failure, RouteOperationFailure, ContainFailure, ReturnEntry under immediate-exit-70), shared by the emission analysis — whose Stage-E flags now gate on it — and by the adapter, which is the verified body's complete emission and carries its hosted-entry-v1 marker; the startup body row names ::main. Corpus 1982 of 2482 (+51; the six OwnedArguments programs wait on their marshaling schedule slice); oracle 57/0/0 at O2-c++23 and O0-c++20. Compiler-generated calls now carry complete schedules through their direct value operands (call-input analysis pool 82 to 19 measured occurrences); the remaining function pools concentrate on the constructor cluster — 52 rollback + 36 constructor checked-CFG + 45 construct-propagation occurrences — plus capability rows 94, ordered-compound 26, body-name rows 23, packs 18. Construct propagation joined the transparent exemption (constructor failure terminates at its own site on every shipped path until the constructor failure ABI exists; function checked-CFG 82 to 35), the rollback diagnostic now names its gap (14 constructor bodies do not route failure edges in MIR — a frontend lowering gap — plus one unarmed field), and member calls on slot-held receivers spell through the slot's checked accessor, caught by cli_workflow's backend-isolation fixture. Edge-free constructor bodies are vacuously rollback-covered (backend-local; the shared routing predicate proved load-bearing for lowering and stays untouched). The Closure/Lambda port is designed: the Closure op spells an inline lambda literal wrapping the recursively emitted Lambda-instance body, matching the compatibility inline form; CallableDispatch follows through the sealed call_parameter template system, then Payload, RawMemory, and VirtualDispatch. 0.208.0 delivers the inline closure chain (after mir-v34's parameter-binding prerequisite at 0.207.0): C++ closure types are unnameable, so no lambda-typed place or value ever declares — a Closure fuses into its consuming invocations, each spelling the complete literal (Capture-row names over enclosing place expressions, positional parameters, the recursively emitted verified lambda body with its own banner marker) under the frozen-capture proof: captured places are written only by entry-block Initializes preceding the Closure, never loaned or dropped, the entry block is never re-entered, and move captures collapse to exactly one direct same-block invocation. Lambda bodies admit in the plain success shape only, so checked arithmetic inside a literal keeps the compatibility terminal helper spelling (::gti_internal::backend::add family — the helper contains the defined failure itself) and the lambda's MIR failure edges plus the invocation's Invoke else-edge are unreachable in text; callable-value invocations pass plain staged values with no CallInput schedule. Rows: Closure cpp_inline_lambda_v1, CallableDispatch cpp_deduced_callable_v1, one never-called body row per lambda instance beside the existing Capture name rows. Corpus 1985 of 2482 (+3: the pure-closure main and its two lambda bodies; the callable pools' remaining yield waits on deduction-called template emission for call_parameter bodies, the recorded next slice); full suite 65/65 with all eight oracle configurations green; owned or lambda-typed captures and every unfused callable shape decline fail-closed. 0.209.0 lands the deduced-callable template vocabulary dormant: Lambda-kind types are now row-free by design (requireType and the row builder skip them — the previous rows carried a forged 'void' spelling nothing could soundly consume), a callable parameter place declares only under a template emission's overlay type row spelling the template parameter name, its Load/Move stages the place for exactly one invocation receiver spelled with no intermediate copy, may-raise callees inside such bodies are exact plain calls (every reachable convention is terminally contained), and the invoke/propagate edges spell as gotos and aborts; the probe declines everything without the overlay row, so production emission is unchanged (corpus 1985 of 2482, suite 65/65) until the declaration-level selector lands. 0.210.0 lands it live: selectedMirCallableTemplateText groups a declaration's monomorphized instances, emits each under its own overlay row (concrete callable type spelled as the declaration's template parameter name), requires byte-identical text with the per-instance banner lines as the only permitted divergence, and emits one template definition — compat header and return type, MIR argument naming — carrying one banner per covered instance. Callers join on both conventions: a fused closure literal spells inline as the deduction call's callable argument, a template body passes its callable parameter place by value, and a deduced-callable callee keeps the plain terminally-contained convention everywhere (transformedCallee excludes it, the failure form calls it plainly with an unconditional invoke edge, and the ADR 017 admission fixpoint no longer drops its callers). Corpus 1991 of 2482 (+6: ex30's whole chain — three templates, main, lambda — plus yield elsewhere); full suite 65/65 with all eight oracle configurations green over the live route; class-callable operator() dispatch (53), owned callable transport (55), and the remaining callable mains stay recorded on task #41. 0.211.0 implements the reference-return failure ABI (ADR 018 §5): a may-raise loan-returning body transforms with a `T **` out-parameter, its Return-with-loan publishes the pointer and returns true, the boundary wrapper dereferences on success, the general-failure selector admits loan-returning declarations, and the caller side pairs a transformed reference-returning call's `T **` out-argument directly with its produced CallResult loan pointer; call-result loans from discharged prefix-storage reads additionally bind their element address at the call itself when no Borrow claims them, and reference-typed values never declare. Proven end to end by a dedicated fixture (transformed member, wrapper, MIR-emitted caller, correct runtime behavior) with the full suite green; the corpus holds at 1991 of 2482 because the string/format cluster (97 ready bodies) stacks further gates the probe now names exactly — StorageBoundsCheck spelling (150 occurrences), checked NumericAliasConversion intrinsic calls (114), non-borrow receiver forms (24), success-form storage operations (15) — the measured next slices of the P-STORAGE-01 backend campaign. 0.212.0 lands the first two: a StorageBoundsCheck call spells the exact compatibility terminal helper (::gti_internal::backend::index_bounds_check over its two staged operands — the helper reports the container's defined GTI-R0007 contract and never returns on failure, so the paired invoke edge is a plain goto on both forms), and a site-carrying numeric conversion arriving as an intrinsic call becomes a failure-form checked detector (mir_checked_convert_v1 status helper writing the converted value, joining the existing record and edge machinery). Corpus 2039 of 2482 (+48: the string/format/loan cluster's first wave), full suite 65/65 with all eight oracle configurations green. Remaining measured gates: non-borrow-staged receiver forms (24), success-form storage operations (15), class-callable dispatch and owned transport (~25), plus the constructor cluster, ordered-compound, packs, and capability pools. 0.213.0 widens the transformed convention to expected-typed results: a may-raise body returning Expected with a scalar or void payload publishes by value through the ordinary out-parameter (selector, probe, and caller rule all agree; the scalar-payload demand keeps the boundary default-constructible on both standards). Measured with the real-form probe census: the corpus holds at 2039 because the format family that motivated the widening decomposes into pack-parameterized bodies (the packs pool), string-payload expecteds (the class-result boundary), and interiors blocked on pack folds — so this increment is enabling machinery that yields with those pools; suite 65/65 with all eight oracle configurations green. The census also sharpened the remaining function-body picture corpus-wide: class-valued failure returns 40, non-borrow receivers ~21, plus small tails. 0.214.0 lands the class-valued publication vocabulary dormant: a failure Return whose class value is a single-use slot-free Construct spells the constructor call inline into the `T *` out-parameter, with the construct silent at its own site — but the selector stays closed because the trial run caught a real boundary defect (cli_workflow/project_cli_workflow: the wrapper's value-initialized boundary and assignment-publication both require default constructibility, and storage_sentinel's deleted default constructor rejected exactly that), so the class slice waits on a placement-new publication design; suite restored to 65/65 with all eight oracle configurations green and corpus at 2039 of 2482. 0.215.0 reopens it soundly: the selector admits a class result only when the semantic lifecycle proves the boundary shape compiles (a usable default constructor and move assignment via findClassLifecycle), which excludes exactly the measured offenders — storage_iterator and storage_sentinel carry deleted members — while admitting string, whose bodies now wait only on their storage interiors. The receiver census is recorded: BorrowRead receivers 36 per cluster example (already vocabulary), Value receivers ~20 (method chains on class-typed values, the ~21-body pool needing call-result chaining), BorrowWrite 1. Suite 65/65 with all eight oracle configurations green; corpus 2039 of 2482. 0.216.0 adds the failure-free logical-length read to both forms — a PrefixStorageLength call spells the shipped prefix_storage_length helper over its staged storage place — another enabling piece of the storage-interior vocabulary; the length-carrying bodies stack further storage gates, so the corpus holds while the pieces accumulate under green suites. 0.222.0 admits direct borrow receivers — a self-member call borrows its own receiver on the receiver operand with no CallInput stage, and both forms spell the place like the staged path — measured by a scratch instrumentation of every probe decline point across the corpus (the receiver rule was the top failure-form blocker at 21 occurrences). The widening surfaced and fixed three latent defects fail-closed: a structural operator bridge cannot carry the transformed sibling suffix (`operator==__gti_mir_failure` is not a name), so bridge-spelled operator bodies decline the failure form while mangled operator names keep transforming; a transformed reference-returning callee's caller loan now declares the callee's return element type instead of the loan source's type (the string-subscript caller spelled `string *` where `const uint8 *` was required); and the probe demands the element row for those loans. Corpus 2043 of 2482; suite 65/65 with all eight oracle configurations green. The decline census ranks what remains: constructor cluster (failure records and non-field initializers), invoke producers outside the checked vocabulary, This-rooted field chains, staging leaks, and the non-general body kinds (hosted-startup schedules, non-passive initializer bodies). 0.223.0 lands the stores-reference constructor schedule (ADR 018): each reference-field initializer pairs bijectively with one Stored loan whose source is the dereference carrier of a reference parameter — cppMirStoredReferenceBindings is the single pairing authority shared by the selector, the probe, and the member-initializer-list writer — the emitted constructor binds the field straight to its C++ reference parameter, the Borrow spells as a comment, no loan pointer materializes, and a nonzero constructor borrow origin is admissible exactly alongside that schedule. The full chain is pinned end-to-end by a non-generic unit fixture (` : target(__gti_mir_arg_0) ` plus the constructor banner); every corpus occurrence sits on a generic owner, so the corpus held at 2043 until constructor emission joined the ADR 019 member-specialization path. 0.224.0 lands exactly that: a constructor of a generic class publishes per admitted concrete instance as the explicit specialization of that constructor — declaration after the class definition, definition beside the deferred template, the stores-reference member initializer list included — with the per-instance and owner-shape eligibility halves extracted into shared helpers (generalConstructorInstanceEligible, generalConstructorOwnerEligible, passiveMirFieldInitializers) so the non-generic selector and the specialization route consult one authority. Corpus 2058 of 2482 (+15: the nine stores-reference constructors and six plain generic-owner constructors); suite 65/65 with all eight oracle configurations green; unit fixtures pin both the non-generic chain and the generic specialization spelling. 0.225.0 admits trivial drops: a Drop whose every governing obligation — the ones its lifecycle events name and the ones anchored on its destination place — carries neither a destructor nor active cleanup spells as a comment, because C++ scope-end destruction of the declared local is exactly the verified semantics; slot-shaped places stay on the lifetime-slot protocol regardless of triviality (the engage/destroy pairing is the verified escape check, and the CLI canaries caught the one attempt to relax it). Enabling only — the trivial-drop bodies stack further gates, the next of which is now decomposed: their invoke producers are Expected unwraps, receiver-borrow calls with failure sites and no function target, the recorded next vocabulary slice. 0.226.0 lands the reference-field receiver chain: a This-rooted place projecting a Field then its Dereference binds through the same live-member reference local the single-field form uses — C++ reference members dereference implicitly — so reference fields read through the receiver join the vocabulary. Corpus 2064 of 2482 (+6, the operator-fixture members); suite 65/65 with all eight oracle configurations green. The census also re-verified against the live tree after an implemented Expected-unwrap slice proved redundant (the tree already carried that vocabulary): remaining probeable blockers are the bridge-operator naming design, storage temporaries with cleanup, residual invoke producers, function-body stored loans, and single constructor/lambda stragglers, with the remaining mass in hosted-startup schedules, non-passive initializers, and the template families. 0.198.0 closed the passive-initializer pools: per-instance generic-owner initializer selection (every instance verified, identical field spellings required), capability/opaque-handle verified-empty initializer markers, and empty module bodies. Remaining pools by measured size (561): 368 functions (219 not-ready — 155 checked-failure control flow, 90 capability rows, 82 call-input schedules, overlapping — plus 94 admission-fixpoint drops, 34 gate-rejected, 23 selector-signature losses), 72 field-initializer + 6 static-field-initializer bodies with staged non-passive schedules, 57 hosted-startup bodies, 42 constructors, 8 lambdas, 5 non-empty module bodies, 3 destructors. | failure-capable `scalar-failure-callgraph-v1` complete | Build general MIR emission for every remaining body and initialization family, validated by the differential oracle, and remove the compatibility execution bridge. | No reachable GTI body or optimization decision executes from AST/HIR; all legacy body and failure helpers are gone; migrated bodies agree with the compatibility emitter or are recorded as not comparable. |
...
| 4 | `S-CFG-01` | **planned** | `M-BACK-02` done | Add a build-configuration axis to compile-time conditionals: valueless `#define`/`#undef`/`#ifdef`/`#ifndef` and boolean conditions over the existing `target.*` primary, with no substitution and no function-like macros. | A fixture selects different branches under different define sets, a `#define` in a dead branch does not leak, an inactive branch with a syntax error still fails, the MIR census is unmoved, and the LSP fades inactive branches. Design in `docs/plans/conditional-compilation.md`; ledger row `R-BUILD-CONFIG`. |
