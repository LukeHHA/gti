...
Checkpoint version: 0.294.0 plus the completed hard-cutover audit, bounded
F1/F2/F3 C-interoperation families, same-thread native callback boundary, and
exact qualified class specialization, mutable field groups, static assertions,
and bounded mutable temporary receivers.
...
- target selection, compiler-owned target/configuration conditionals, one GTI-owned scalar
